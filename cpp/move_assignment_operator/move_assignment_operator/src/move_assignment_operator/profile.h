...
class class_profile{ // 簡易名簿
  ...
  public: // 外部からアクセス可.
    ...
    class_profile(class_profile &&obj); // ムーブコンストラクタclass_profile(class_profile &&obj).
    ~class_profile(); // デストラクタ~class_profile().
    class_profile &operator=(const class_profile &obj); // コピー代入演算子
    class_profile &operator=(class_profile &&obj); // ムーブ代入演算子
