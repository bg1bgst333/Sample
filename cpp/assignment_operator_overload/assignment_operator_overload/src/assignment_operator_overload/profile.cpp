...
// 代入演算子のオーバーロードoperator=(const class_profile &obj)
class_profile& class_profile::operator=(const class_profile &obj){

  // set_profileで値をセットする.(すでに値がセットされていた場合はいったんメモリを解放してから, 再確保するので安全.)
  set_profile(obj.name_, obj.age_, obj.address_); // set_profileでobj.name_, obj.age_, obj.address_をセット.

  // 代入演算子のオーバーロードが呼ばれたことを示す.
  std::cout << "assignment_operator_overload" << std::endl; // "assignment_operator_overload"と出力.
...
void class_profile::set_profile(const char *name, const int age, const char *address){
  ...
  // すでにセットされている場合はメモリの解放.
  if (name_ != NULL){ // name_がNULLでなければ.
    delete [] name_; // delete[]でname_を解放.
    name_ = NULL; // name_にNULLを格納.
  }
  if (address_ != NULL){ // address_がNULLでなければ.
    delete [] address_; // delete[]でaddress_を解放.
    address_ = NULL; // address_にNULLを格納.
  }
