...
// メンバ関数output()
void class_profile::output(){ // name_, age_, address_を一斉出力.
...
class class_detail : public class_profile{ // 詳細名簿
  ...
  public: // このクラスの外部からもアクセス可.
    ...
    void output_detail(); // 基底クラスの要素も含めて一斉出力.
...
// メンバ関数output_detail()
void class_detail::output_detail(){ // name_, age_, address_, に加え, phonenumber, sex, height, weightも一斉出力.

  // name_, age_, address_の出力.
  output(); // outputでname_, age_, address_を出力.
...
int main(){
  ...
  class_detail det; // class_detailのオブジェクトdetを宣言.
  ...
  // prof(name_, age_, address_)の一斉出力.
  prof.output(); // outputでname_, age_, address_を出力.
  ...
  det.output_detail(); // output_detailでname_, age_, address_, phonenumber_, sex_, height_, weight_を出力.
