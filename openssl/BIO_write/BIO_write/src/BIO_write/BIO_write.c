...
int main(void){
  ...
  /* PUTリクエスト文字列の作成. */
  strcat(request_str, "PUT /share/test.txt HTTP/1.1\r\n"); /* strcatでrequest_strに"PUT /share/test.txt HTTP/1.1\r\n"を連結. */
  strcat(request_str, "Host: bgstation0.com:80\r\n"); /* strcatでrequest_strに"Host: bgstation0.com:80\r\n"を連結. */
  strcat(request_str, "Content-Length: 9\r\n"); /* strcatでrequest_strに"Content-Length: 9\r\n"を連結. */
  strcat(request_str, "Content-Type: text/plain\r\n"); /* strcatでrequest_strに"Content-Type: text/plain\r\n"を連結. */
  strcat(request_str, "\r\n"); /* strcatでrequest_strに"\r\n"を連結. */
  strcat(request_str, "ABCDE\nXYZ"); /* strcatでrequest_strに"ABCDE\nXYZ"を連結. */
