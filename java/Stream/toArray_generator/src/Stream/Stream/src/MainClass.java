...
public class MainClass{	// MainClassの定義
	...
	public static void main(String[] args){	// mainメソッドの定義
		...
		// filterで絞込みしてmapで名前を返しtoArrayでString配列に変換する.
		String[] list = bodies.stream().filter(x -> x.getHeight() >= 170).map(x -> x.getName()).toArray(x -> new String[x]);	// getHeightが170以上のgetNameで取得した名前のString配列に変換.
		for (String s : list){	// listの分繰り返す.
