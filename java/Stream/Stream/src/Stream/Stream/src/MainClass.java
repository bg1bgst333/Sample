...
public class MainClass{	// MainClassの定義
	...
	public static void main(String[] args){	// mainメソッドの定義
		...
		// Streamを取得.
		Stream<Body> sbody = bodies.stream();	// Stream<Body>インスタンスsbodyを取得.
		Object[] list = sbody.filter(x -> x.getHeight() > 173).map(x -> x.getName()).toArray();	// x.getHeightで173を超える要素から, x.getNameで名前を取得し, 配列に変換.
