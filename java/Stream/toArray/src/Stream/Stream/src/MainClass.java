...
public class MainClass{	// MainClassの定義
	...
	public static void main(String[] args){	// mainメソッドの定義
		...
		// filterで絞込みしてmapで名前を返しtoArrayで配列に変換する.
		Object[] list = bodies.stream().filter(x -> x.getHeight() >= 170).map(x -> x.getName()).toArray();	// getHeightが170以上のgetNameで取得した名前の配列に変換.(Object[]になってしまう.)
		for (Object s : list){	// listの分繰り返す.
			String name = (String)s;	// ObjectをStringにキャスト.
