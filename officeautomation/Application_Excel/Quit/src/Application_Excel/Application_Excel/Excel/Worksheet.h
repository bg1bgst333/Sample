...
namespace Excel {
	...
	class CWorksheet {
		...
		public:
			...
			CRange * get_Cells(int row, int column);	// Cellsプロパティ(Rangeオブジェクト)の取得.(row, columnを渡す.)
			CRange * get_Range(LPCTSTR lpctszCell);	// Rangeプロパティの取得.
			CRange * get_Range(CRange * range1, CRange * range2);	// Rangeプロパティの取得.(range1, range2を渡す.)
