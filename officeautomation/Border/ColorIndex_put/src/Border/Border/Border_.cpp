...
int _tmain(int argc, TCHAR *argv[]) {	// main関数のTCHAR版.
	...
	// A1の右罫線を赤にする.
	Excel::CBorder * pBorderA1Right = NULL;	// CBorderポインタpBorderA1RightをNULLで初期化.
	pBorderA1Right = pBordersA1->get_Item(Excel::XlBordersIndex::xlEdgeRight);	// Excel::CBorders::get_ItemでpBorderA1Rightを取得.
	_tprintf(_T("CBorder * pBorderA1Right = 0x%08x\n"), (unsigned long)pBorderA1Right);	// pBorderA1Rightを出力.
	pBorderA1Right->put_LineStyle(Excel::XlLineStyle::xlContinuous);	// Excel::CBorder::put_LineStyleで罫線スタイルを実線に.
	pBorderA1Right->put_Weight(Excel::XlBorderWeight::xlThin);	// Excel::CBorder::put_Weightで罫線太さを極細に.
	pBorderA1Right->put_ColorIndex(3);	// Excel::CBorder::put_ColorIndexで3(赤)に.
	...
	// B2:C3の下罫線を青にする.
	Excel::CBorder * pBorderB2C3Bottom = NULL;	// CBorderポインタpBorderB2C3BottomをNULLで初期化.
	pBorderB2C3Bottom = pBordersB2C3->get_Item(Excel::XlBordersIndex::xlEdgeBottom);	// Excel::CBorders::get_ItemでpBorderB2C3Bottomを取得.
	_tprintf(_T("CBorder * pBorderB2C3Bottom = 0x%08x\n"), (unsigned long)pBorderB2C3Bottom);	// pBorderB2C3Bottomを出力.
	pBorderB2C3Bottom->put_LineStyle(Excel::XlLineStyle::xlDash);	// Excel::CBorder::put_LineStyleで罫線スタイルを破線に.
	pBorderB2C3Bottom->put_Weight(Excel::XlBorderWeight::xlMedium);	// Excel::CBorder::put_Weightで罫線太さを普通に.
	pBorderB2C3Bottom->put_ColorIndex(5);	// Excel::CBorder::put_ColorIndexで5(青)に.

	// pBorderB2C3Bottomの破棄.
	if (pBorderB2C3Bottom != NULL) {
		delete pBorderB2C3Bottom;
	}

	// pBorderA1Rightの破棄.
	if (pBorderA1Right != NULL) {
		delete pBorderA1Right;
	}
