...
// Itemプロパティ(Rangeオブジェクト)の取得.(strRowを渡す.)
Excel::CRange * Excel::CRange::get_Item(LPCTSTR strRow) {
	...
	DISPID dispidname = DISPID_PROPERTYPUT;	// dispidnameをDISPID_PROPERTYPUTに初期化.
	var.vt = VT_BSTR;	// 文字列
	var.bstrVal = SysAllocString(strRow);	// SysAllocStringでstrRowのBSTR生成.
	VariantInit(&varResult);	// VariantInitでvarResult初期化.
...
// Bordersオブジェクトプロパティの取得.
Excel::CBorders * Excel::CRange::get_Borders() {

	// メソッドやプロパティのDISPIDの取得.
	LPOLESTR lpwszName = L"Borders";	// lpwszNameを"Borders"で初期化.
	DISPID dispid;	// DISPID型dispid.
	HRESULT hr = m_pDispatch->GetIDsOfNames(IID_NULL, &lpwszName, 1, LOCALE_USER_DEFAULT, &dispid);	// IDispatch::GetIDsOfNamesでdispid取得.
	if (FAILED(hr)) {	// 失敗.
		return NULL;
	...
	// 成功.
	CBorders *pBorders = new CBorders(var.pdispVal);	// CBordersのコンストラクタにvar.pdispValをセット.
	return pBorders;

}

// Rangeオブジェクトプロパティの取得.
Excel::CRange * Excel::CRange::get_Range(LPCTSTR strRange) {

	// メソッドやプロパティのDISPIDの取得.
	LPOLESTR lpwszName = L"Range";	// lpwszNameを"Borders"で初期化.
	DISPID dispid;	// DISPID型dispid.
	HRESULT hr = m_pDispatch->GetIDsOfNames(IID_NULL, &lpwszName, 1, LOCALE_USER_DEFAULT, &dispid);	// IDispatch::GetIDsOfNamesでdispid取得.
	if (FAILED(hr)) {	// 失敗.
		return NULL;
	...
	DISPID dispidname = DISPID_PROPERTYPUT;	// dispidnameをDISPID_PROPERTYPUTに初期化.
	var.vt = VT_BSTR;	// 文字列
	var.bstrVal = SysAllocString(strRange);	// SysAllocStringでstrRangeのBSTR生成.
	VariantInit(&varResult);	// VariantInitでvarResult初期化.
	dispparams.cArgs = 1;	// 引数の数は1.
	dispparams.rgvarg = &var;	// &varを指定.
	dispparams.rgdispidNamedArgs = NULL;	// NULLを指定.
