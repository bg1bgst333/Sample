...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	// ショートカットを保存.
	TCHAR tszShortCutPath[MAX_PATH] = {0};	// tszShortCutPath(長さMAX_PATH)を{0}で初期化.
	GetModuleFileName(NULL, tszShortCutPath, MAX_PATH);	// GetModuleFileNameでこの実行ファイルのフルパスを取得.
	PathRemoveFileSpec(tszShortCutPath);	// PathRemoveFileSpecでファイル名の部分を取り除く.
	_tcscat(tszShortCutPath, _T("\\ShortCut.lnk"));	// 上記パスに"\ShortCut.lnk"を連結.
	pPersistFile->Save(tszShortCutPath, TRUE);	// pPersistFile->Saveで保存.

	// ショートカットのパスを出力.
	_tprintf(_T("tszShortCutPath = %s\n"), tszShortCutPath);	// tszShortCutPathを出力.
