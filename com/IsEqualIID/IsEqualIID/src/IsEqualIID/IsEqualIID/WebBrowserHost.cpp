...
STDMETHODIMP CWebBrowserHost::QueryInterface(const IID &riid, void **ppvObject){
	*ppvObject = NULL;
	...
	// 以下のIIDの比較検証.
	static const IID IID_ITes11 = {0x12345678, 0x1234, 0x5678, {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}};
	static const IID IID_ITes12 = {0x12345678, 0x1234, 0x5678, {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}};
	IID iid1 = IID_ITes11;
	IID iid2 = IID_ITes12;
	// "=="で比較.
	if (iid1 == iid2){
		OutputDebugString(_T("iid1 == iid2\n"));
	}
	else{
		OutputDebugString(_T("iid1 != iid2\n"));
	}
	// IsEqualIIDで比較.
	if (IsEqualIID(iid1, iid2)){
		OutputDebugString(_T("IsEqualIID(&iid1, &iid2) is TRUE!\n"));
	}
	else{
		OutputDebugString(_T("IsEqualIID(&iid1, &iid2) is FALSE!\n"));
	}
