...
int _tmain(int argc, TCHAR *argv[]) {	// main関数のTCHAR版.
	...
	WCHAR wszGuid[256];	// WCHAR文字配列wszGuid(長さ256)
	...
	// GUIDを文字列に変換.
	StringFromGUID2(pTypeAttr->guid, wszGuid, 256);	// StringFromGUID2でpTypeAttr->guidを文字列に変換し, wszGuidに格納.

	// GUID文字列を出力.
	_tprintf(_T("pTypeAttr->guid = %s\n"), wszGuid);	// wszGuidを出力.
