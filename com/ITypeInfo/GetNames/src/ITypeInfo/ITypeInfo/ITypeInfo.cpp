...
int _tmain(int argc, TCHAR *argv[]) {	// main関数のTCHAR版.
	...
	FUNCDESC *pFuncDesc;	// FUNCDESC構造体ポインタpFuncDesc.
	UINT uiSize;	// サイズuiSize.
	...
	// 各FUNCDESCを取得.
	for (int i = 0; i < pTypeAttr->cFuncs; i++) {	// pTypeAttr->cFuncsの数だけ繰り返す.
		pTypeInfo->GetFuncDesc(i, &pFuncDesc);	// ITypeInfo::GetFuncDescでFUNCDESCを取得.
		BSTR strFuncName;	// 関数名ポインタBSTR文字列strFuncName.
		pTypeInfo->GetNames(pFuncDesc->memid, &strFuncName, 1, &uiSize);	// ITypeInfo::GetNamesで関数名を取得.
		_tprintf(_T("strFuncName = %s\n"), strFuncName);	// strFuncNameを出力.
		SysFreeString(strFuncName);	// SysFreeStringでstrFuncNameを解放.
		pTypeInfo->ReleaseFuncDesc(pFuncDesc);	// ITypeInfo::ReleaseFuncDescで解放.
	}
