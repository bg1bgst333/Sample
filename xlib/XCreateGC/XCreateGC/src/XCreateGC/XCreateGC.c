...
int main(int argc, char **argv){
  ...
  XColor screen_in_out; /* 色情報screen_in_out */
  ...
  /* 青のピクセル値を取得. */
  screen_in_out.red = 0x0; /* redは0x0. */
  screen_in_out.green = 0x0; /* greenは0x0. */
  screen_in_out.blue = 0xffff; /* blueは0xffff. */
  XAllocColor(d, default_colormap, &screen_in_out); /* XAllocColorにRGB値を設定したsreen_in_outを指定するとピクセル値が格納される. */

  /* 白のピクセル値を取得. */
  white = XWhitePixel(d, 0); /* XWhitePixelでスクリーン0における白のピクセル値を取得し, whiteに格納. */

  /* ウィンドウの生成. */
  wr = XCreateSimpleWindow(d, DefaultRootWindow(d), 100, 100, 640, 480, 1, white, screen_in_out.pixel); /* XCreateSimpleWindowでウィンドウ生成し, 結果はwrに格納.(screen_in_out.pixelにピクセル値が格納されている.) */
