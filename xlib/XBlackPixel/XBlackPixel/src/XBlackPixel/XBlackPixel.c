...
int main(void){
  ...
  /* 黒のRGB値を取得. */
  black = XBlackPixel(d, 0); /* XBlackPixelでスクリーン0における黒のRGB値を取得し, blackに格納. */

  /* ウィンドウの生成. */
  wr = XCreateSimpleWindow(d, RootWindow(d, 0), 100, 100, 640, 480, 1, white, black); /* XCreateSimpleWindowでウィンドウ生成し, 結果はwrに格納. */
