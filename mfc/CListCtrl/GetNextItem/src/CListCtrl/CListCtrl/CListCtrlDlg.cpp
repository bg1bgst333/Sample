...
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	// 選択されているアイテムのインデックス(行)を取得.
	int idx = -1;		// インデックスidxを-1で初期化.
	TCHAR tszIdx[32];	// idxを文字列に変換したものを格納するバッファtszIdx.(長さ32)
	CListCtrl *pListCtrl1 = (CListCtrl *)GetDlgItem(IDC_LIST1);	// GetDlgItemでリストコントロールIDC_LIST1のオブジェクトポインタをpListCtrl1に格納.((CListCtrl *)にキャスト.)
	while ((idx = pListCtrl1->GetNextItem(idx, LVNI_SELECTED)) != -1){	// pListCtrl1->GetNextItemで条件に合う指定したidxの次のインデックスが戻り値に返るのでidxに格納.(LVNI_SELECTEDは選択された項目という条件.idxは最初-1にして呼ぶ.)
		_stprintf(tszIdx, _T("idx = %d"), idx);	// _stprintfでidxを文字列に変換し, tszIdxに格納.
		AfxMessageBox(tszIdx, MB_OK | MB_ICONASTERISK);	// AfxMessageBoxでtszIdxを表示.
	}
}
