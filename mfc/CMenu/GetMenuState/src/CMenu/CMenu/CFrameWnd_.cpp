...
void CFrameWnd_::OnPaint(){	// 画面の描画を要求された時.
	...
	if (!m_bInitMenu){	// FALSEなら初期化.
		...
		pSubMenu->EnableMenuItem(ID_ITEM_1_1, MF_BYCOMMAND | MF_GRAYED);	// pSubMenu->EnableMenuItemでID_ITEM_1_1をMF_GRAYEDにする.
		pSubMenu->EnableMenuItem(ID_ITEM_1_2, MF_BYCOMMAND | MF_ENABLED);	// pSubMenu->EnableMenuItemでID_ITEM_1_2をMF_ENABLEDにする.
		pSubMenu->EnableMenuItem(ID_ITEM_1_3, MF_BYCOMMAND | MF_DISABLED);	// pSubMenu->EnableMenuItemでID_ITEM_1_3をMF_DISABLEDにする.
