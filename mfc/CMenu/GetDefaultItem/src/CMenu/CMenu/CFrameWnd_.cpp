...
void CFrameWnd_::OnItem1_1(){	// メニュー項目Item1_1のコマンドハンドラ
	...
	pSubMenu->SetDefaultItem(ID_ITEM_1_2);	// pSubMenu->SetDefaultItemでID_ITEM_1_2を既定の項目にする.
...
void CFrameWnd_::OnItem1_3(){	// メニュー項目Item1_3のコマンドハンドラ
	...
	CMenu *pMenu = GetMenu();	// メニューの取得.
	CMenu *pSubMenu = pMenu->GetSubMenu(0);	// 0番目のサブメニュー取得.
	UINT uiId = pSubMenu->GetDefaultItem(0);	// pSubMenu->GetDefaultItemで既定の項目のリソースID取得.
	if (uiId == ID_ITEM_1_1){
		AfxMessageBox(_T("ID_ITEM_1_1"));
	}
	else if (uiId == ID_ITEM_1_2){
		AfxMessageBox(_T("ID_ITEM_1_2"));
	}
	else if (uiId == ID_ITEM_1_3){
		AfxMessageBox(_T("ID_ITEM_1_3"));
	}
	else if (uiId == ID_ITEM_1_4){
