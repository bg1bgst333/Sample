...
void CFrameWnd_::OnItem1_1(){	// メニュー項目Item1_1のコマンドハンドラ
	...
	// 0番目のサブメニュー項目の0番目の項目のテキストを取得.
	CMenu *pMenu = GetMenu();	// メニューの取得.
	CMenu *pSubMenu = pMenu->GetSubMenu(0);	// 0番目のサブメニュー取得.
	TCHAR tszItem[16] = {0};	// TCHAR文字配列tszItem(要素数16)を{0}で初期化.
	pSubMenu->GetMenuString(0, tszItem, 15, MF_BYPOSITION);	// pSubMenu->GetMenuStringで0番目のテキスト取得.
	AfxMessageBox(tszItem);	// tszItemを表示.
