================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: _AfxDispatchCmdMsg プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した _AfxDispatchCmdMsg には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには _AfxDispatchCmdMsg アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

_AfxDispatchCmdMsg.vcproj
...
_AfxDispatchCmdMsg.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   C_AfxDispatchCmdMsgApp アプリケーション クラスを宣言します。

_AfxDispatchCmdMsg.cpp
   これは、アプリケーション クラス C_AfxDispatchCmdMsgApp を含むメイン アプリケー
   ション ソース ファイルです。

_AfxDispatchCmdMsg.rc
...
res\_AfxDispatchCmdMsg.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル _AfxDispatchCmdMsg.rc に含まれていま
   す。

res\_AfxDispatchCmdMsg.rc2
...
_AfxDispatchCmdMsgDoc.h, _AfxDispatchCmdMsgDoc.cpp - ドキュメント
   これらのファイルは C_AfxDispatchCmdMsgDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via C_AfxDispatchCmdMsgDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

_AfxDispatchCmdMsgView.h, _AfxDispatchCmdMsgView.cpp - ドキュメントのビュー
   これらのファイルは C_AfxDispatchCmdMsgView クラスを含みます。
   C_AfxDispatchCmdMsgView オブジェクトは C_AfxDispatchCmdMsgDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   _AfxDispatchCmdMsg.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
