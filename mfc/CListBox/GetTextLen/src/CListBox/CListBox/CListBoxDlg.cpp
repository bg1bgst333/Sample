...
void CListBoxDlg::OnBnClickedButton2()
{
	...
	TCHAR tszLen[32];	// テキストの長さを文字列にするために用意したバッファtszLen.(長さ32)
	_stprintf(tszLen, _T("len = %d"), len);	// _stprintfでlenを文字列に変換しtszLenに格納.
	AfxMessageBox(tszLen, MB_OK | MB_ICONASTERISK);	// AfxMessageBoxでtszLenを表示.
}
