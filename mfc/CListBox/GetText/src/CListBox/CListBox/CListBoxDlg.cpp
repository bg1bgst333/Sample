...
void CListBoxDlg::OnBnClickedButton2()
{
	...
	CListBox *pListBox1 = (CListBox *)GetDlgItem(IDC_LIST1);	// GetDlgItemでリストボックスコントロールIDC_LIST1のオブジェクトポインタをpListBox1に格納.((CListBox *)にキャスト.)
	CEdit *pEdit1 = (CEdit *)GetDlgItem(IDC_EDIT1);	// GetDlgItemでエディットボックスコントロールIDC_EDIT1のオブジェクトポインタをpEdit1に格納.((CEdit *)にキャスト.)
	int idx = pListBox1->GetCurSel();		// pListBox1->GetCurSelで選択されているインデックスを取得.
	int len = pListBox1->GetTextLen(idx);	// pListBox1->GetTextLenで選択されているテキストの長さを取得.
	TCHAR *ptszText = new TCHAR[len + 1];	// 長さlen + 1のバッファptszTextを用意.
	pListBox1->GetText(idx, ptszText);	// pListBox1->GetTextで選択されたテキストを取得.
	pEdit1->SetWindowText(ptszText);	// pEdit1->SetWindowTextでエディットボックスにptszTextをセット.
	delete [] ptszText;	// ptszText解放.
}
