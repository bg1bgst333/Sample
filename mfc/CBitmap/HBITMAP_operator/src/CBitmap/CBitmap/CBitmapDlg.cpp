...
BOOL CBitmapDlg::OnInitDialog()
{
	...
	if (bRet){	// 成功.
		...
		HBITMAP hBitmap = (HBITMAP)(*pBitmap);	// HBITMAP演算子により, ビットマップハンドルhBitmapを取得.
		TCHAR tszHBitmap[16] = {0};	// TCHARバッファtszHBitmap(長さ16)を{0}で初期化.
		_stprintf(tszHBitmap, _T("%08x"), hBitmap);	// hBitmapをtszHBitmapに変換.
		AfxMessageBox(tszHBitmap);	// tszHBitmapを表示.
	}
	else{	// 失敗.
