================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: AfxFindMessageEntry プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した AfxFindMessageEntry には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには AfxFindMessageEntry アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

AfxFindMessageEntry.vcproj
...
AfxFindMessageEntry.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   CAfxFindMessageEntryApp アプリケーション クラスを宣言します。

AfxFindMessageEntry.cpp
   これは、アプリケーション クラス CAfxFindMessageEntryApp を含むメイン アプリケー
   ション ソース ファイルです。

AfxFindMessageEntry.rc
...
res\AfxFindMessageEntry.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル AfxFindMessageEntry.rc に含まれていま
   す。

res\AfxFindMessageEntry.rc2
...
AfxFindMessageEntryDoc.h, AfxFindMessageEntryDoc.cpp - ドキュメント
   これらのファイルは CAfxFindMessageEntryDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CAfxFindMessageEntryDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

AfxFindMessageEntryView.h, AfxFindMessageEntryView.cpp - ドキュメントのビュー
   これらのファイルは CAfxFindMessageEntryView クラスを含みます。
   CAfxFindMessageEntryView オブジェクトは CAfxFindMessageEntryDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   AfxFindMessageEntry.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
