...
BOOL CComboBoxDlg::OnInitDialog()
{
	...
	// あらかじめ, "あいうえお", "かきくけこ", "さしすせそ"を追加しておく.
	...
	pComboBox1->AddString(_T("あいうえお"));	// pComboBox1->AddStringでリスト領域に"あいうえお"を追加する.
	pComboBox1->AddString(_T("かきくけこ"));	// pComboBox1->AddStringでリスト領域に"かきくけこ"を追加する.
	pComboBox1->AddString(_T("さしすせそ"));	// pComboBox1->AddStringでリスト領域に"さしすせそ"を追加する.

	// デフォルトで"さしすせそ"にセットしておく.
	pComboBox1->SetCurSel(2);	// pComboBox1->SetCurSelで0から数えて2番目をセット.
...
{
	// TODO: ここにコントロール通知ハンドラ コードを追加します。
	// 選択した項目のインデックスをメッセージボックスで表示.
	CComboBox *pComboBox1 = (CComboBox *)GetDlgItem(IDC_COMBO1);	// GetDlgItemでコンボボックスコントロールIDC_COMBO1のオブジェクトポインタをpComboBox1に格納.((CComboBox *)にキャスト.)
	int idx = pComboBox1->GetCurSel();	// pComboBox1->GetCurSelで選択された項目のインデックスを取得.
	TCHAR *ptszIdx = new TCHAR[64];	// 長さ64のバッファを確保.
	_stprintf(ptszIdx, _T("idx = %d"), idx);	// idxを文字列に変換.	
	AfxMessageBox(ptszIdx, MB_OK | MB_ICONASTERISK);	// AfxMessageBoxでptszIdxを出力.
	delete [] ptszIdx;	// ptszIdxを解放.
}
