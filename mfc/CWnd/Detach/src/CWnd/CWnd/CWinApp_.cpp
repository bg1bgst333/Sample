...
BOOL CWinApp_::InitInstance(){	// インスタンスの初期化処理
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CWnd");							// ウィンドウクラス名は"CWnd".
	...
	wc.lpszMenuName = MAKEINTRESOURCE(IDM_MAINMENU);		// メニューIDはIDM_MAINMENU.
	...
	// CFrameWnd_の作成
	pFrameWnd_->Create(_T("CWnd"), _T("CWnd"));	// CreateでCFrameWnd_のインスタンスのウィンドウを作成.
