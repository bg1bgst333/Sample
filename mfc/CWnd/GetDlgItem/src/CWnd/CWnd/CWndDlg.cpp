...
void CWndDlg::OnBnClickedButton1()
{
	...
	CWnd *pButton1 = GetDlgItem(IDC_BUTTON1);	// Cwnd::GetDlgItemでボタンIDC_BUTTON1のオブジェクトポインタpButton1を取得.
	HWND hButton1 = ::GetDlgItem(m_hWnd, IDC_BUTTON1);	// Win32APIの::GetDlgItemでボタンIDC_BUTTON1のウィンドウハンドルhButton1を取得.
	if (pButton1->m_hWnd == hButton1){	// 同じなら.
		AfxMessageBox(_T("pButton1->m_hWnd == hButton1"));	// "pButton1->m_hWnd == hButton1"と表示.
	}
}
