...
void CWndDlg::OnBnClickedButton1()
{
	...
	int iLen = pEdit1->GetWindowTextLength();	// CWnd::GetWindowTextLengthでテキストの長さを取得.
	TCHAR tszLen[16] = {0};	// TCHAR配列tszLen(長さ16)を{0}で初期化. 
	_stprintf(tszLen, _T("%d"), iLen);	// iLenをtszLenに変換.
	AfxMessageBox(tszLen);	// tszLenを表示.
}
