...
void CFrameWnd_::OnItem1_1(){	// メニュー項目Item1_1のコマンドハンドラ
	...
	// オブジェクトの宣言
	CDialog_ dlg_(IDD_DIALOG);	// CDialog_オブジェクトdlg_の宣言(リソースIDD_DIALOGを元に作成.)

	// ダイアログの表示.
	INT_PTR nRes = dlg_.DoModal();	// DoModalメンバ関数でモーダルダイアログとして表示.
	if (nRes == IDOK){	// "OK"(ダイアログ上のどこかをクリックした)時.
		AfxMessageBox(_T("OK"), MB_OK | MB_ICONASTERISK);	// AfxMessageBoxで"OK"と表示.
	}
	else if (nRes == IDCANCEL){	// "キャンセル"(閉じるボタンを押した)時.
