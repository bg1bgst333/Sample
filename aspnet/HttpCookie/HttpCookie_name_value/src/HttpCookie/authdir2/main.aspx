...
<html>
    <head runat="server">
        <script runat="server">
            ...
            {
                HttpCookie cookie = new HttpCookie("first", "abcde");
            ...
            <div>
                <%
                  if (reqCookies.Count > 0)
                  {
                      ...
                      if (reqCookies["authdir2"] != null)
                      {
                          Response.Write("reqCookies[\"authdir2\"].Value = " + reqCookies["authdir2"].Value + "<br />");
                      }
                      if (reqCookies["second"] != null)
                      {
                          Response.Write("reqCookies[\"second\"].Value = " + reqCookies["second"].Value);
                      }
                  }
                %>
            </div>
