...
if ($q->request_method eq 'POST'){ # 'POST'の時.
  ...
  # クッキー作成.
  my $cookie = $q->cookie(-name => 'sessid', -value => $session->id, -expires => '+5m');

  # HTTPヘッダーの出力.
  print $q->header(-cookie => $cookie);

  # HTMLヘッダの出力.
  print $q->start_html("CGI::Session::load(dsn, sid)"); # <html>タグなどの出力.

  # 表示.
  print "username = ".$q->param('username').$q->br;
  print "userpass = ".$q->param('userpass').$q->br;
  print "sessid = ".$session->id.$q->br;
  ...
  my $sid = $q->cookie('sessid');
  my $session = CGI::Session->load(undef, $sid);

  # 期限切れクッキー作成.
  my $expired = $q->cookie(-name => 'sessid', -value => $session->id, -expires => '-1d');
  ...
  # セッションIDの表示.
  if ($session){
    print "load sessid: ".$session->id.$q->br;
  }
