...
public class MainActivity extends Activity implements SurfaceHolder.Callback, Camera.ShutterCallback, Camera.PictureCallback{
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height){
    	...
    	parameters.setPictureSize(parameters.getPreviewSize().width, parameters.getPreviewSize().height);	// ピクチャーサイズにプレビューサイズをセット.
	...
	public void onPictureTaken(byte[] data, Camera camera) {
		// TODO Auto-generated method stub
    	// 撮影する画像のサイズを表示.
    	Toast.makeText(this, "width = " + String.valueOf(camera.getParameters().getPictureSize().width) + ", height = " + String.valueOf(camera.getParameters().getPictureSize().height), Toast.LENGTH_LONG).show();	// widthとheightを表示.
		// dataをファイルに保存.
