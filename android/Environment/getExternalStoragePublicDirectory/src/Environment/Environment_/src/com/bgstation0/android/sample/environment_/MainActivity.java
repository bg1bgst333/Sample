    		...
    		case R.id.menu_write_file:
    			...
    			try{	// tryで囲む.
    				...
    				if (Environment.MEDIA_MOUNTED.equals(state)){	// MEDIA_MOUNTEDなら.
    					...
    					File ext_downloads_dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);	// Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)でDownloadディレクトリext_downloads_dirを取得.
    					File ext_file = new File(ext_downloads_dir, filename);	// filenameのFileオブジェクトext_fileを生成.
