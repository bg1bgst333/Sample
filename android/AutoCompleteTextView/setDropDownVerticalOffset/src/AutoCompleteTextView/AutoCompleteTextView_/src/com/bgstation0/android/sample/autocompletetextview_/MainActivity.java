...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // SearchableInfoから閾値を取得.
        int threshold = searchableInfo.getSuggestThreshold();	// thresholdを取得.
        //Toast.makeText(mContext, "searchableInfo threshold = " + String.valueOf(threshold), Toast.LENGTH_LONG).show();	// thresholdを表示.
        ...
        // オートコンプリートのサジェスト閾値を設定.
        autoCompleteTextView.setThreshold(threshold);	// 閾値にthresholdをセット.

        // サジェストの表示位置を調整.
        autoCompleteTextView.setDropDownVerticalOffset(50);	// 50ぐらい下に下げる.(単位がdpなのかは不明.)
        
        // 入力がなくても, サジェストを表示するようにする.
        autoCompleteTextView.setOnFocusChangeListener(new View.OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				...
				// フォーカスが取れたら表示.
				if (hasFocus){	// hasFocusがtrue.
					autoCompleteTextView.showDropDown();	// autoCompleteTextView.showDropDownでサジェスト表示.
				}
