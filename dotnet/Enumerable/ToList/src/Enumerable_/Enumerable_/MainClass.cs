...
class MainClass // MainClassの定義
{
    ...
    static void Main()  // Mainメソッドの定義
    {
        ...
        // LINQで身長170以上のボディの名前を取得.
        List<string> result = bodies.Where(x => x.Height >= 170).Select(x => x.Name).ToList();    // 身長170以上の名前のListを取得.
        foreach (string s in result)   // foreachでresultからsを取り出す.
        {
            Console.WriteLine(s);   // sの各要素を出力.
        }
