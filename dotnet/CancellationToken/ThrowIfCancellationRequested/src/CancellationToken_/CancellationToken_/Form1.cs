    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var t = Task.Factory.StartNew(() => // Task.Factory.StartNew(action, cancellationToken)で実行.
            {
                ...
                {
                    token.ThrowIfCancellationRequested();   // token.ThrowIfCancellationRequestedでcts.Cancelが実行されたら例外投げる.
                    Debug.WriteLine("Step..."); // "Step..."とデバッグ出力.
                    Thread.Sleep(1000); // 1秒休止.
                }
            }, token);  // tokenも渡す.
            t.ContinueWith((x) => // Task.ContinueWithでtの継続タスクを指定.
            {
                Debug.WriteLine("Finish!"); // "Finish!"とデバッグ出力.
            });
