...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CB_GETCOUNT");						// ウィンドウクラス名は"CB_GETCOUNT".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("CB_GETCOUNT"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("CB_GETCOUNT"), _T("CB_GETCOUNT"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"CB_GETCOUNT"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("CB_GETCOUNT"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				// アイテム追加
				SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)_T("あ"));
				SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)_T("いい"));
				SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)_T("ううう"));
				SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)_T("ええええ"));
				SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)_T("おおおおお"));

				// 最初のアイテムで選択.
				SendMessage(hCombo, CB_SETCURSEL, (WPARAM)0, 0);

				// 全てのアイテムテキストを列挙.
				for (int i = 0; i < SendMessage(hCombo, CB_GETCOUNT, 0, 0); i++){
					int iLen = SendMessage(hCombo, CB_GETLBTEXTLEN, (WPARAM)i, 0);
					TCHAR *ptszText = new TCHAR[iLen + 1];
					SendMessage(hCombo, CB_GETLBTEXT, (WPARAM)i, (LPARAM)ptszText);
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case IDC_BUTTON1:
						...
						// IDC_BUTTON1ブロック
						{
							...
							// コンボボックスのエディット部分のテキストを取得.
							int iLen = GetWindowTextLength(hCombo);
							if (iLen > 0){	// 0より大きい.
								TCHAR *ptszText = new TCHAR[iLen + 1];
								ZeroMemory(ptszText, (iLen + 1) * sizeof(TCHAR));
								GetWindowText(hCombo, ptszText, iLen + 1);

								// 末尾にテキストを追加
								SendMessage(hCombo, CB_ADDSTRING, (WPARAM)0, (LPARAM)ptszText);	// CB_SETCURSELで末尾にテキストを追加.
