...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_HSCROLL");					// ウィンドウクラス名は"WM_HSCROLL".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("WM_HSCROLL"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_HSCROLL"), _T("WM_HSCROLL"), WS_OVERLAPPEDWINDOW | WS_HSCROLL | WS_VSCROLL, 0, 0, 480, 360, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WM_HSCROLL"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("WM_HSCROLL"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	static SCROLLINFO scrHorz = {0};	// 水平方向スクロール情報scrHorzを{0}で初期化.
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("WM_HSCROLL"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
				...
				if (hDC == NULL){	// NULLなら.
					...
					// エラー処理
					MessageBox(hwnd, _T("GetDC failed!"), _T("WM_HSCROLL"), MB_OK | MB_ICONHAND);	// MessageBoxで"GetDC failed!"とエラーメッセージを表示.
				...
				if (hMemDC == NULL){	// NULLでなければ.
					...
					// エラー処理
					MessageBox(hwnd, _T("CreateCompatibleDC failed!"), _T("WM_HSCROLL"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateCompatibleDC failed!"とエラーメッセージを表示.
			...
			// WM_HSCROLLブロック
			{
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)に行われた操作の通知コードが格納されているので, それで判定する.
					...
					case SB_LINELEFT:
						...
						// 1列左に戻す.
						if (scrHorz.nPos > 0){	// scrHorz.nPosが1以上なら.
							scrHorz.nPos--;	// scrHorz.nPosを減らす.
							// 変化量だけスクロール.
							ScrollWindow(hwnd, 1, 0, NULL, NULL);	// ScrollWindowでスクロール.
						}
						break;	// 抜ける.
					...
					case SB_LINERIGHT:
						...
						// 1列右に進める.
						if (scrHorz.nPos < scrHorz.nMax - (int)scrHorz.nPage + 1){	// scrHorz.nPosが最後の位置より前なら.
							scrHorz.nPos++;	// scrHorz.nPosを増やす.
							// 変化量だけスクロール.
							ScrollWindow(hwnd, -1, 0, NULL, NULL);	// ScrollWindowでスクロール.
						}
						break;	// 抜ける.
					...
					case SB_PAGELEFT:
						...
						// SB_PAGELEFTブロック.
						{
							...
							// 1ページ戻る.	
							int pos = scrHorz.nPos;	// 以前の位置
							int page = scrHorz.nPage;	// ページサイズ
							int hMax = scrHorz.nMax;	// 最大値
							...
							if (after <= f){	// ページサイズを引いた時の位置が0以下.
								...
								scrHorz.nPos -= page;	// ページサイズを引く.
								d = page;	// ページサイズが変化量.
							}
							// 変化量だけスクロール.
							ScrollWindow(hwnd, d, 0, NULL, NULL);	// ScrollWindowでスクロール.
					...
					case SB_PAGERIGHT:
						...
						// SB_PAGERIGHTブロック.
						{
							...
							int f = hMax - page + 1;	// 実際の最後の位置.
							int d = 0;	// 変化量.
							if (after >= f){	// ページサイズを足した時の位置が, 実際の最後の位置を超えている.
								scrHorz.nPos = f;	// 実際の最後の位置をセット.
								d = f - pos;	// 最後の位置から以前の位置までが変化量.
							}
							else{	// 超えていない.
								scrHorz.nPos += page;	// ページサイズを足す.
								d = page;	// ページサイズが変化量.
							}
							// 変化量だけスクロール.
							ScrollWindow(hwnd, -1 * d, 0, NULL, NULL);	// ScrollWindowでスクロール.
					...
					case SB_THUMBPOSITION:
						...
						// SB_THUMBPOSITIONブロック.
						{
							...
							// 変化量だけスクロール.
							ScrollWindow(hwnd, before - after, 0, NULL, NULL);	// ScrollWindowでスクロール.
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押された時.(uMsgがWM_LBUTTONDOWNの時.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				scrHorz.nPage = rc.right - rc.left + 1;	// ページサイズはrc.right - rc.left + 1.
				scrHorz.fMask = SIF_PAGE | SIF_RANGE;	// ページ, レンジをセット.
				SetScrollInfo(hwnd, SB_HORZ, &scrHorz, TRUE);	// スクロール情報をセット.
				scrHorz.nPos = 0;	// 現在位置は0.
				scrHorz.fMask = SIF_POS;	// 位置をセット.
				SetScrollInfo(hwnd, SB_HORZ, &scrHorz, TRUE);	// スクロール情報をセット.
