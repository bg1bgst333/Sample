...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_DragMove");				// ウィンドウクラス名は"ImageList_DragMove".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ってきたらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ImageList_DragMove"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_DragMove"), _T("ImageList_DragMove"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ImageList_DragMove"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ImageList_DragMove"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
		...
		case WM_CREATE:		// ウィンドウの作成が開始されたとき.(uMsgがWM_CREATEの場合.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_DragMove"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
					return -1;	// ウィンドウ作成失敗.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_DragMove"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
				...
				// ドラッグ画像の移動を制限するエリアを, ウィンドウの中央に設定.
				{
					...
					// ドラッグ画像(32x32, ホットスポットは左上の(0, 0))がrcLimitからはみ出さないよう,
					// ホットスポットの可動範囲rcHotspotLimitは, 右辺・下辺を画像サイズ(32px)分だけ内側に縮める.
			...
			// WM_MOUSEMOVEブロック
			{
				...
				if (bDragging){
					...
					if (!PtInRect(&rcHotspotLimit, pt)){	// rcHotspotLimit外に出ようとした場合.
						...
						// これにより, 画像自体の端がちょうどrcLimitの枠線に揃う.
					...
					// 左上のスクリーン座標を引くことで, ウィンドウ基準の座標に変換する.
					POINT ptWin = pt;
					...
					ImageList_DragMove(ptWin.x, ptWin.y);	// ImageList_DragMoveでドラッグ画像を(ptWin.x, ptWin.y)(ウィンドウ基準座標)の位置に移動.
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				if (PtInRect(&rcLimit, pt)){
					...
					if (ImageList_BeginDrag(hImageList, 0, 0, 0)){	// ImageList_BeginDragでhImageListの0番目の画像をドラッグ開始. ホットスポットは画像左上(0, 0).
						...
						// ImageList_DragEnterも, hwndの左上角(タイトルバー・枠を含む)を基準にした座標を要求する.
						POINT ptWin = pt;
						ClientToScreen(hwnd, &ptWin);	// ClientToScreenでクライアント座標からスクリーン座標に変換.
						...
						if (ImageList_DragEnter(hwnd, ptWin.x, ptWin.y)){	// ImageList_DragEnterでhwndをロックしてドラッグ画像をクリック位置(ptWin)(ウィンドウ基準座標)に表示.
