...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("DEVNAMES");						// ウィンドウクラス名は"DEVNAMES".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("DEVNAMES"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("DEVNAMES"), _T("DEVNAMES"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"DEVNAMES"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("DEVNAMES"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_PRINT:
						...
						// ID_ITEM_PRINTブロック
						{
							...
							psd.rtMargin.left = 500;	// 左端マージン500.
							psd.rtMargin.top = 500;		// 上端マージン500.
							psd.rtMargin.right = 500;	// 右端マージン500.
							psd.rtMargin.bottom = 500;	// 下端マージン500.
							psd.hDevMode = GlobalAlloc(GHND, sizeof(DEVMODE));	// GlobalAllocでDEVMODEのハンドルメモリを確保.

							// DEMOVDEの初期設定
							DEVMODE *pDevMode = (DEVMODE *)GlobalLock(psd.hDevMode);	// GlobalLockでpsd.hDevModeをロック.
							pDevMode->dmSize = sizeof(DEVMODE);	// DEVMODEのサイズをセット.(これがないと変更が反映されない.)
							pDevMode->dmSpecVersion = DM_SPECVERSION;	// Windows10だとこれがないと変更が反映されない.
							pDevMode->dmFields |= DM_ORIENTATION;	// 変更対象は印刷方向.
							pDevMode->dmOrientation |= DMORIENT_LANDSCAPE;	// 印刷方向を横にセット.
							GlobalUnlock(psd.hDevMode);	// GlobalUnlockでアンロック.

							// PageSetupDlgの表示.
							BOOL bRet = PageSetupDlg(&psd);	// PageSetupDlgにpsdを渡して表示し, 戻り値をBOOLのbRetに格納.
							if (bRet){	// bRetがTRUE.
								...
								pd.hDevMode = psd.hDevMode;	// pd.hDevModeにpsd.hDevModeを渡す.
								pd.hDevNames = psd.hDevNames;	// pd.hDevNamesにpsd.hDevNamesを渡す.

								// ドライバ名, デバイス名, 出力名の取得, 既定のプリンタかどうか表示.
								DEVNAMES *pDevNames = (DEVNAMES *)GlobalLock(pd.hDevNames);	// pDevNames取得.
								TCHAR *ptszDriverName = (TCHAR *)((BYTE *)pDevNames + (pDevNames->wDriverOffset * sizeof(TCHAR)));	// ドライバ名の位置に調整.
								MessageBox(hwnd, ptszDriverName, _T("DEVNAMES"), MB_OK);	// ドライバ名表示.
								TCHAR *ptszDeviceName = (TCHAR *)((BYTE *)pDevNames + (pDevNames->wDeviceOffset * sizeof(TCHAR)));	// デバイス名の位置に調整.
								MessageBox(hwnd, ptszDeviceName, _T("DEVNAMES"), MB_OK);	// デバイス名表示.
								TCHAR *ptszOutputName = (TCHAR *)((BYTE *)pDevNames + (pDevNames->wOutputOffset * sizeof(TCHAR)));	// 出力名の位置に調整.
								MessageBox(hwnd, ptszOutputName, _T("DEVNAMES"), MB_OK);	// 出力名表示.
								if (pDevNames->wDefault == DN_DEFAULTPRN){	// 既定のプリンタの場合.
									MessageBox(hwnd, _T("DN_DEFAULTPRN"), _T("DEVNAMES"), MB_OK);	// "DN_DEFAULTPRN"と表示.
								}
								GlobalUnlock(pd.hDevNames);	// アンロック.

								// PrintDlgの表示.
								bRet = PrintDlg(&pd);	// PrintDlgにpdを渡して表示し, 戻り値をbRetに格納.
								if (bRet){	// 印刷を押したらTRUE.
									...
									di.lpszDocName = _T("DEVNAMES");	// ドキュメント名は"DEVNAMES"とする.
									iRetSD = StartDoc(pd.hDC, &di);	// StartDocで印刷ジョブの開始.
									if (iRetSD > 0){	// 印刷開始成功.
										iRetSP = StartPage(pd.hDC);	// StartPageでページの開始.
										if (iRetSP > 0){	// ページ開始成功.
											TextOut(pd.hDC, 50, 50, _T("ABCDE"), _tcslen(_T("ABCDE")));	// TextOutでプリンタの(50, 50)の位置に"ABCDE"を描画.
							...
							// 終了処理.
							GlobalFree(psd.hDevMode);	// psd.hDevModeを解放.
