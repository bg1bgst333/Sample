...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_CTLCOLOR");					// ウィンドウクラス名は"WM_CTLCOLOR".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("WM_CTLCOLOR"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_CTLCOLOR"), _T("WM_CTLCOLOR"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WM_CTLCOLOR"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("WM_CTLCOLOR"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_CTLCOLOREDIT:
			...
			// WM_CTLCOLOREDITブロック
			{
				...
				// Win32ではもう呼ばれないので自前で呼ぶ.
				return SendMessage(hwndDlg, WM_CTLCOLOR, wParam, lParam);
		...
		// ダイアログの子コントロールの背景の描画を要求された時.(Win16専用)
		case WM_CTLCOLOR:
			...
			// WM_CTLCOLORブロック
			{
				...
				// ブラシハンドル作成.
				if (hbrEditBackground == NULL){
					hbrEditBackground = CreateSolidBrush(RGB(0, 0, 255));	// 青
				}
				...
				if (hwndCtrl == hEdit1){	// IDC_EDIT1
					SetBkColor(hDC, RGB(0, 0, 255));	// 青
					SetTextColor(hDC, RGB(255, 255, 255));	// 白
					return (INT_PTR)hbrEditBackground;	// 青のブラシを返す.
				}
				else{
