...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_MEASUREITEM");					// ウィンドウクラス名"WM_MEASUREITEM".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_MEASUREITEM"), _T("WM_MEASUREITEM"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
		...
		case WM_DRAWITEM:
			...
			// WM_DRAWITEMブロック
			{
				...
				if (lpDIS->CtlType == ODT_BUTTON){
					...
					// "ＭＳ 明朝"フォントを新規に作成.
					HDC hDC = lpDIS->hDC;
					HFONT hFont = CreateFont(32, 0, 0, 0, FW_REGULAR, FALSE, FALSE, FALSE, SHIFTJIS_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("ＭＳ 明朝"));	// CreateFontで"ＭＳ 明朝"フォントのGDIオブジェクトを新規に作成し, hFontに格納.(設定はデフォルトのものにしておく.)
					HFONT hOld = (HFONT)SelectObject(hDC, hFont);
					if (lpDIS->itemData == 0){
						TextOut(hDC, 0, 0, _T("あいうえお"), lstrlen(_T("あいうえお")));
					}
					else if (lpDIS->itemData == 1){
						TextOut(hDC, 0, 50, _T("かきくけこ"), lstrlen(_T("かきくけこ")));
					}
					else if (lpDIS->itemData == 2){
						TextOut(hDC, 0, 100, _T("さしすせそ"), lstrlen(_T("さしすせそ")));
					}
