...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_AddMasked");				// ウィンドウクラス名"ImageList_AddMasked".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_AddMasked"), _T("ImageList_AddMasked"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap1 == NULL){	// hBitmap1がNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_AddMasked"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_AddMasked"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
