...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetMenuItemCount");				// ウィンドウクラス名は"GetMenuItemCount".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetMenuItemCount"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetMenuItemCount"), _T("GetMenuItemCount"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"GetMenuItemCount"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetMenuItemCount"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				AppendMenu(hPopupMenu1, MF_ENABLED | MF_STRING, ID_ITEM_1_1, _T("Item1-1(&C)"));	// AppendMenuでhPopupMenu1にメニュー項目Item1-1を追加.
				AppendMenu(hPopupMenu1, MF_ENABLED | MF_STRING, ID_ITEM_1_2, _T("Item1-2(&D)"));	// AppendMenuでhPopupMenu1にメニュー項目Item1-2を追加.
				AppendMenu(hPopupMenu1, MF_ENABLED | MF_STRING, ID_ITEM_1_3, _T("Item1-3(&E)"));	// AppendMenuでhPopupMenu1にメニュー項目Item1-3を追加.

				// ポップアップメニューhPopupMenu2にメニュー項目を追加.
				AppendMenu(hPopupMenu2, MF_ENABLED | MF_STRING, ID_ITEM_2_1, _T("Item2-1(&F)"));	// AppendMenuでhPopupMenu2にメニュー項目Item2-1を追加.
		...
		case WM_DESTROY:	// ウィンドウが破棄された時.(uMsgがWM_DESTROYの時.)
			...
			// WM_DESTROYブロック
			{
				...
				// ポップアップメニュー項目の削除.
				DeleteMenu(hPopupMenu2, ID_ITEM_2_1, MF_BYCOMMAND);	// DeleteMenuでID_ITEM_2_1を削除.
				...
				// メニュー項目の削除.
				DeleteMenu(hMenu, ID_ITEM_2, MF_BYCOMMAND);	// DeleteMenuでID_ITEM_2を削除.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							// hPopupMenu1のメニュー項目の数を取得.
							int iCount = GetMenuItemCount(hPopupMenu1);	// GetMenuItemCountでhPopupMenu1のメニュー項目の数を取得.
							TCHAR tszCount[128] = {0};	// TCHAR型文字配列tszCount(長さ128)を{0}で初期化.
							_stprintf(tszCount, _T("iCount = %d"), iCount);	// _stprintfでiCountをtszCountに変換.
							MessageBox(hwnd, tszCount, _T("GetMenuItemCount"), MB_OK | MB_ICONASTERISK);	// MessageBoxでtszCountを表示.
