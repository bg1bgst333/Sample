...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GET_X_LPARAM");						// ウィンドウクラス名は"GET_X_LPARAM".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GET_X_LPARAM"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GET_X_LPARAM"), _T("GET_X_LPARAM"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GET_X_LPARAM"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GET_X_LPARAM"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case NM_CLICK:	// クリック.
							...
							// NM_CLICKブロック.
							{
								...
								// 最後のメッセージの時のカーソル位置のX座標を出力.
								DWORD dwPos = GetMessagePos();	// GetMessagePosで最後のメッセージのカーソル位置を取得.
								short x = GET_X_LPARAM(dwPos);
								TCHAR tszX[64] = {0};
								_stprintf(tszX, _T("NM_CLICK: x = %hd\n"), x);
								OutputDebugString(tszX);	// OutputDebugStringでtszXを出力.
