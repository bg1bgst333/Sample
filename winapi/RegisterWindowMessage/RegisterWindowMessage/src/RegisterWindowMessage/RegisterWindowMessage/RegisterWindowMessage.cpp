...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("RegisterWindowMessage");				// ウィンドウクラス名"RegisterWindowMessage".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("RegisterWindowMessage"), _T("RegisterWindowMessage"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	// 登録済みメッセージが来た場合.
	if (uMsg == uMessage){
		MessageBox(hwnd, _T("FINDMSGSTRING"), _T("RegisterWindowMessage"), MB_OK);
		return 0;	// 0を返す.
	}
