...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ResetDC");						// ウィンドウクラス名は"ResetDC".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ResetDC"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("ResetDC"), _T("ResetDC"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ResetDC"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ResetDC"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_PRINT:
						...
						// ID_ITEM_PRINTブロック
						{
							...
							DEVMODE dmDevMode = {0};	// DEVMODE構造体dmDevModeを{0}で初期化.
							...
							if (hPrinterDC != NULL){	// hPrinterDCがNULLでなければ.
								// 印刷設定変更
								dmDevMode.dmSize = sizeof(DEVMODE);	// DEVMODEのサイズをセット.(これがないと変更が反映されない.)
								dmDevMode.dmSpecVersion = DM_SPECVERSION;	// Windows10だとこれがないと変更が反映されない.
								dmDevMode.dmFields |= DM_ORIENTATION;	// 変更対象は印刷方向.
								dmDevMode.dmOrientation |= DMORIENT_LANDSCAPE;	// 印刷方向を横にセット.
								hPrinterDC = ResetDC(hPrinterDC, &dmDevMode);	// ResetDCでdmDevModeを反映して更新後のhPrinterDCを取得.
								// 印刷開始
