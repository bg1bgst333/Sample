...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetDlgItem");						// ウィンドウクラス名は"GetDlgItem".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetDlgItem"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetDlgItem"), tszWindowName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetDlgItem"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetDlgItem"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-2"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-2"と表示.
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-3"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-3"と表示.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-1"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-1"と表示.
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-2"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-2"と表示.
					...
					case ID_ITEM_2_3:
						...
						// ID_ITEM_2_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-3"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-3"と表示.
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-1"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-1"と表示.
					...
					case ID_ITEM_3_2:
						...
						// ID_ITEM_3_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-2"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-2"と表示.
					...
					case ID_ITEM_3_3:
						...
						// ID_ITEM_3_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-3"), _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-3"と表示.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON1:
						...
						// ID_BUTTON1ブロック
						{
							...
							// 変数・配列の宣言
							HWND hButton;	// ボタンのハンドルhButton.
							HWND hStatic;	// スタティックコントロールのハンドルhStatic.
							TCHAR tszHButton[128];	// ボタンのハンドルの値を文字列にしたものtszHButton.
							TCHAR tszHStatic[128];	// スタティックコントロールのハンドルの値を文字列にしたものtszHStatic.

							// このダイアログ上のコントロールのリソースIDを取得.
							hButton = GetDlgItem(hwndDlg, ID_BUTTON1);	// GetDlgItemでID_BUTTON1のハンドルを取得.
							hStatic = GetDlgItem(hwndDlg, ID_STATIC1);	// GetDlgItemでID_STATIC1のハンドルを取得.

							// 文字列変換
							_stprintf(tszHButton, _T("hButton = %08x"), hButton);	// _stprintfでhButtonを文字列に変換.
							_stprintf(tszHStatic, _T("hStatic = %08x"), hStatic);	// _stprintfでhStaticを文字列に変換.

							// 表示.
							MessageBox(NULL, tszHButton, _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxでtszHButtonを表示.
							MessageBox(NULL, tszHStatic, _T("GetDlgItem"), MB_OK | MB_ICONASTERISK);	// MessageBoxでtszHStaticを表示.
