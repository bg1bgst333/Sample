...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CB_GETLBTEXT");						// ウィンドウクラス名は"CB_GETLBTEXT".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("CB_GETLBTEXT"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("CB_GETLBTEXT"), _T("CB_GETLBTEXT"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"CB_GETLBTEXT"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("CB_GETLBTEXT"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				// 0から数えて2番目の長さを取得し, 表示.
				int iLen = SendMessage(hCombo, CB_GETLBTEXTLEN, (WPARAM)2, 0);
				TCHAR tszLen[32] = {0};
				_stprintf(tszLen, _T("iLen = %d\n"), iLen);
				OutputDebugString(tszLen);

				// 2番目のテキストも取得し, 表示.
				TCHAR *ptszText = new TCHAR[iLen + 1];
				SendMessage(hCombo, CB_GETLBTEXT, (WPARAM)2, (LPARAM)ptszText);
