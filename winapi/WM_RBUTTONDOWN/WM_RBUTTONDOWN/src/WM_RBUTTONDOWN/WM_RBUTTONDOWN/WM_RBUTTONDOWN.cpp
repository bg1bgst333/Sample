...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_RBUTTONDOWN");				// ウィンドウクラス名は"WM_RBUTTONDOWN".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("WM_RBUTTONDOWN"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}
 
	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_RBUTTONDOWN"), _T("WM_RBUTTONDOWN"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WM_RBUTTONDOWN"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("WM_RBUTTONDOWN"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	static int x;	// マウス右ボタンが押された時のマウスカーソルの位置x座標.
	static int y;	// マウス右ボタンが押された時のマウスカーソルの位置y座標.
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_PAINT:		// 画面の描画を要求された時.(uMsgがWM_PAINTの時.)
			...
			// WM_PAINTブロック
			{
				...
				TCHAR tszXY[128];	// マウス右ボタンが押された時のマウスカーソル位置座標を表示するために用意したTCHAR型文字配列tszXY.(長さ128)
		...
		case WM_RBUTTONDOWN:	// マウスの右ボタンが押された時.(uMsgがWM_RBUTTONDOWNの時.)
			...
			// WM_RBUTTONDOWNブロック
			{
				...
				// マウスの右ボタンが押されたので, 画面を無効領域化することで更新を促す.
