...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetMenuItemID");					// ウィンドウクラス名は"GetMenuItemID".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetMenuItemID"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetMenuItemID"), _T("GetMenuItemID"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"GetMenuItemID"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetMenuItemID"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_1:
						...
						// ID_ITEM_1_1ブロック
						{
							...
							// Item1のIDを取得.
							UINT id = GetMenuItemID(hPopupMenu1, 0);	// GetMenuItemIDで0番目のidを取得.
							if (id == ID_ITEM_1_1){	// idがID_ITEM_1_1なら.
								MessageBox(hwnd, _T("id == ID_ITEM_1_1"), _T("GetMenuItemID"), MB_OK);	// "id == ID_ITEM_1_1"と表示.
							}
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// Item2のIDを取得.
							UINT id = GetMenuItemID(hPopupMenu1, 1);	// GetMenuItemIDで1番目のidを取得.
							if (id == ID_ITEM_1_2){	// idがID_ITEM_1_2なら.
								MessageBox(hwnd, _T("id == ID_ITEM_1_2"), _T("GetMenuItemID"), MB_OK);	// "id == ID_ITEM_1_2"と表示.
							}
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// Item3のIDを取得.
							UINT id = GetMenuItemID(hPopupMenu1, 2);	// GetMenuItemIDで2番目のidを取得.
							if (id == ID_ITEM_1_3){	// idがID_ITEM_1_3なら.
								MessageBox(hwnd, _T("id == ID_ITEM_1_3"), _T("GetMenuItemID"), MB_OK);	// "id == ID_ITEM_1_3"と表示.
							}
