...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	hFile = FindFirstFile(_T("test*"), &wfd);	// FindFirstFileで"test*"に当てはまるファイルを探し, 最初に見つかったファイル情報をwfdに格納.
	if (hFile == INVALID_HANDLE_VALUE){	// INVALID_HANDLE_VALUEなら失敗.
	...
	// ハンドルを閉じる.
	FindClose(hFile);	// FindCloseでhFileを閉じる.
