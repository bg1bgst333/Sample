...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_EndDrag");				// ウィンドウクラス名は"ImageList_EndDrag".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ってきたらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ImageList_EndDrag"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_EndDrag"), _T("ImageList_EndDrag"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ImageList_EndDrag"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ImageList_EndDrag"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理ができるように定義したウィンドウプロシージャ.
	...
	static int nDragCount = 0;				// ドラッグを開始した回数を数えるint型スタティック変数nDragCount.(ImageList_EndDragを呼ばなくても複数回ドラッグできるか実験するためのカウンタ.)
	...
	switch (uMsg){	// switch-case文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始されたとき.(uMsgがWM_CREATEの場合.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_EndDrag"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
					return -1;	// ウィンドウ作成失敗.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_EndDrag"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
		...
		case WM_PAINT:		// 画面の描画が求められたとき.(uMsgがWM_PAINTの場合.)
			...
			// WM_PAINTブロック
			{
				...
				TCHAR tszBuf[64];	// nDragCountを表示するための文字列バッファtszBuf.
				...
				// nDragCount(ドラッグ開始回数)を表示. ImageList_EndDragを呼ばなくても連続ドラッグできるか目視確認するため.
				wsprintf(tszBuf, _T("DragCount = %d"), nDragCount);	// wsprintfでtszBufに"DragCount = 数値"の文字列を作成.
				TextOut(hDC, rcLimit.left, rcLimit.bottom + 10, tszBuf, lstrlen(tszBuf));	// TextOutでtszBufをrcLimitの下に表示.
			...
			// WM_MOUSEMOVEブロック
			{
				...
				if (bDragging){
					...
					if (!PtInRect(&rcHotspotLimit, pt)){	// rcHotspotLimit外に出ようとした場合.
						...
						// これにより, 画像自体の端がちょうどrcLimitの枠内に沿う.
					...
					// ImageList_DragMoveは, ImageList_DragEnterで指定したウィンドウ(hwnd)の
					// 左上角(タイトルバー・枠を含む)を基準にした座標を要求する(クライアント座標ではない).
					// ClientToScreenでスクリーン座標に変換したうえで, GetWindowRectで取得したウィンドウの
					// 左上のスクリーン座標を引くことで, ウィンドウ上の座標に変換する.
					POINT ptWin = pt;
				...
				if (PtInRect(&rcLimit, pt)){
					...
					if (ImageList_BeginDrag(hImageList, 0, 0, 0)){	// ImageList_BeginDragでhImageListの0番目の画像をドラッグ開始. ホットスポットは画像左上(0, 0).
						...
						if (ImageList_DragEnter(hwnd, ptWin.x, ptWin.y)){	// ImageList_DragEnterでhwndをロックしてドラッグ画像をクリック位置(ptWin)(ウィンドウ内座標)に表示.
							...
							nDragCount++;			// nDragCountをインクリメント.(ドラッグ開始回数を記録.)
							InvalidateRect(hwnd, NULL, TRUE);	// InvalidateRectで再描画要求(nDragCountの表示更新のため.)
						}
						else{	// ImageList_DragEnterが失敗した場合.
							ImageList_EndDrag();	// ImageList_EndDragでドラッグを終了.(BeginDragしたぶんは必ずEndDragで後始末する.)
						}
					}
