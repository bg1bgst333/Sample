...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LB_SETCURSEL");						// ウィンドウクラス名"LB_SETCURSEL".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("LB_SETCURSEL"), _T("LB_SETCURSEL"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
							...
							switch (HIWORD(wParam)){
								...
								case LBN_DBLCLK:

									{
										...
										// ダブルクリックされたアイテム文字列を取得し, デバッグ出力.
										TCHAR *ptszStr = new TCHAR[len + 1];
										ZeroMemory(ptszStr, sizeof(TCHAR) * (len + 1));
										SendMessage(hListBox1, LB_GETTEXT, idx, (LPARAM)ptszStr);
										TCHAR tszTemp[512] = {0};
										_stprintf(tszTemp, _T("ID_LISTBOX1 LB_GETTEXT ptszStr = %s\n"), ptszStr);
										OutputDebugString(tszTemp);
										// ダブルクリックされたアイテム文字列をID_LISTBOX2に追加し, 選択状態に.
										HWND hListBox2 = GetDlgItem(hwnd, ID_LISTBOX2);
										int idx2 = SendMessage(hListBox2, LB_ADDSTRING, 0, (LPARAM)ptszStr);
										SendMessage(hListBox2, LB_SETCURSEL, idx2, 0);
										delete [] ptszStr;
