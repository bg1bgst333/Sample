...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("MF_BYCOMMAND");					// ウィンドウクラス名は"MF_BYCOMMAND".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("MF_BYCOMMAND"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("MF_BYCOMMAND"), _T("MF_BYCOMMAND"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"MF_BYCOMMAND"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("MF_BYCOMMAND"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_1:
						...
						// ID_ITEM_1_1ブロック
						{
							...
							// Item3-1を削除.
							DeleteMenu(hPopupMenu3, ID_ITEM_3_1, MF_BYCOMMAND);	// DeleteMenuでhPopupMenu3のIDがID_ITEM_3_1となるアイテムを削除.
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// Item3-2を削除.
							DeleteMenu(hPopupMenu3, ID_ITEM_3_2, MF_BYCOMMAND);	// DeleteMenuでhPopupMenu3のIDがID_ITEM_3_2となるアイテムを削除.
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// Item3-3を削除.
							DeleteMenu(hPopupMenu3, ID_ITEM_3_3, MF_BYCOMMAND);	// DeleteMenuでhPopupMenu3のIDがID_ITEM_3_3となるアイテムを削除.
