...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("SB_SIMPLE");						// ウィンドウクラス名"SB_SIMPLE".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("SB_SIMPLE"), _T("SB_SIMPLE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
	...
	switch (uMsg){	// uMsgごとに処理振り分け.
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				int size[] = {100, 250, 450};	// 区画サイズ(100, 150, 200)に分ける場合, こう({100, 250, 450})書く.
				...
				SendMessage(hStatusBar, SB_SETTEXT, (WPARAM)(255 | 0), (LPARAM)_T("ABCDE"));

				// 非シンプルモードでは3つの区画に分ける.
				SendMessage(hStatusBar, SB_SIMPLE, FALSE, 0);
				SendMessage(hStatusBar, SB_SETPARTS, (WPARAM)3, (LPARAM)(LPINT)size);	// SB_SETPARTSで区画分け.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_SETTEXT:
						...
						// ID_ITEM_SETTEXTブロック
						{
							...
							SendMessage(hStatusBar, SB_SETTEXT, (WPARAM)2 | 0, (LPARAM)_T("たちつてと"));	// SB_SETTEXTでhStatusBarに2番目に"たちつてと"をセット.
					...
					// Set Simpleが選択された時.
					case ID_ITEM_SETSIMPLE:

						// ID_ITEM_SETSIMPLEブロック
						{
							...
							if (!bSimple){
								...
								SendMessage(hStatusBar, SB_SIMPLE, (WPARAM)FALSE, 0);
								bSimple = FALSE;
							}
