...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LBN_KILLFOCUS");						// ウィンドウクラス名"LBN_KILLFOCUS".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("LBN_KILLFOCUS"), _T("LBN_KILLFOCUS"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
	...
	switch (uMsg){	// uMsgごとに処理振り分け.
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				// ID_LISTBOX1の作成.
				hListBox1 = CreateWindow(WC_LISTBOX, _T(""), WS_CHILD | WS_VISIBLE | WS_BORDER, 50, 50, 120, 360, hwnd, (HMENU)ID_LISTBOX1, lpCS->hInstance, NULL);	// // CreateWindowでID_LISTBOX1を作成.
				if (hListBox1 == NULL){
					return -1;
				}
				...
				// ID_LISTBOX2の作成.
				hListBox2 = CreateWindow(WC_LISTBOX, _T(""), WS_CHILD | WS_VISIBLE | WS_BORDER, 180, 50, 120, 360, hwnd, (HMENU)ID_LISTBOX2, lpCS->hInstance, NULL);	// // CreateWindowでID_LISTBOX2を作成.
				if (hListBox2 == NULL){
					return -1;
				}
