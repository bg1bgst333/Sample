	...
	switch (uMsg) {	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				HBRUSH hbrBackground = NULL;	// ブラシハンドルhbrBackgroundをNULLで初期化.

				// 黒ブラシの取得.
				hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);	// GetStockObjectでBLACK_BRUSHを取得し, hbrBackgroundに格納.

				// 黒ブラシのセット.
				SetClassLong(hwnd, GCL_HBRBACKGROUND, (LONG)hbrBackground);	// SetClassLongでhwndが指すウィンドウを含めたウィンドウクラスの背景ブラシにhbrBackgroundをセット.
