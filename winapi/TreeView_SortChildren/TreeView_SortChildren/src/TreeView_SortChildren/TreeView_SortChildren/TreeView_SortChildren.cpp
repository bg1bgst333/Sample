...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("TreeView_SortChildren");				// ウィンドウクラス名は"TreeView_SortChildren".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("TreeView_SortChildren"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("TreeView_SortChildren"), _T("TreeView_SortChildren"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"TreeView_SortChildren"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("TreeView_SortChildren"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				HTREEITEM hChild2 = NULL;	// 子要素ハンドルhChild2をNULLで初期化.
				HTREEITEM hChild3 = NULL;	// 子要素ハンドルhChild3をNULLで初期化.
				HTREEITEM hChild4 = NULL;	// 子要素ハンドルhChild4をNULLで初期化.
				...
				// 1番目の子要素追加.
				tvis.hParent = hParent1;	// hParent1が親.
				tvis.item.pszText = _T("Juujiro");	// "Juujiro".
				tvis.item.iImage = 1;	// アイコン1番.
				tvis.item.iSelectedImage = 1;	// アイコン1番.
				hChild2 = (HTREEITEM)TreeView_InsertItem(hTree, &tvis);		// TreeView_InsertItemでtvisの要素を挿入.
				// 1番目の子要素追加.
				tvis.hParent = hParent1;	// hParent1が親.
				tvis.item.pszText = _T("Juuzaburo");	// "Juuzaburo".
				tvis.item.iImage = 1;	// アイコン1番.
				tvis.item.iSelectedImage = 1;	// アイコン1番.
				hChild3 = (HTREEITEM)TreeView_InsertItem(hTree, &tvis);		// TreeView_InsertItemでtvisの要素を挿入.
				// 1番目の子要素追加.
				tvis.hParent = hParent1;	// hParent1が親.
				tvis.item.pszText = _T("Juushiro");	// "Juushiro".
				tvis.item.iImage = 1;	// アイコン1番.
				tvis.item.iSelectedImage = 1;	// アイコン1番.
				hChild4 = (HTREEITEM)TreeView_InsertItem(hTree, &tvis);		// TreeView_InsertItemでtvisの要素を挿入.
					...
					switch (tvi->hdr.code){	// 通知コードごとに分ける.
						...
						case TVN_ENDLABELEDIT:	// 編集終了.
							...
							// TVN_ENDLABELEDITブロック
							{
								...
								// 編集後のテキスト含めた情報をアイテムにセット.
								TreeView_SetItem(hTree, &tvi->item);	// TreeView_SetItemでアイテムにセット.
		...
		case WM_CONTEXTMENU:
			...
			// WM_CONTEXTMENUブロック
			{
				...
				ScreenToClient(hTree, &tvhti.pt);	// ScreenToClientで座標変換.
				TCHAR tszClientXY[256] = {0};
				_stprintf(tszClientXY, _T("Client: x = %d, y = %d\n"), tvhti.pt.x, tvhti.pt.y);
				OutputDebugString(tszClientXY);

				// どのアイテムが右クリックされたか調べる.
				HTREEITEM hItem = TreeView_HitTest(hTree, &tvhti);	// TreeView_HitTestで右クリックされたアイテムのハンドルを取得.
				if (hItem != NULL){	// どれかが選択されている.

					// 右クリックされたアイテムを選択.
					TreeView_SelectItem(hTree, hItem);	// TreeView_SelectItemで 右クリックされたアイテムを選択.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON2:
						...
						// ID_BUTTON2ブロック
						{
							...
							// 追加した項目が表示されるように反映.
							TreeView_EnsureVisible(hTree, hNewItem);	// TreeView_EnsureVisibleで追加した項目が表示されるように反映.
					...
					case ID_DELETE_ITEM:
						...
						// ID_BUTTON3ブロック
						{
							...
							// 選択されたツリービューアイテムハンドルの取得.
							HTREEITEM hItem = TreeView_GetSelection(hTree);	// TreeView_GetSelectionでhItemを取得.

							// 選択されたアイテムの削除.
							TreeView_DeleteItem(hTree, hItem);	// TreeView_DeleteItemでhItemの削除.
