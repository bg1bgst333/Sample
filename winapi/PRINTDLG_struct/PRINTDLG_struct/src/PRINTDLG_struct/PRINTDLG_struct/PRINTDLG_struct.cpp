...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("PRINTDLG_struct");						// ウィンドウクラス名は"PRINTDLG_struct".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("PRINTDLG_struct"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("PRINTDLG_struct"), _T("PRINTDLG_struct"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"PRINTDLG_struct"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("PRINTDLG_struct"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_PRINT:
						...
						// ID_ITEM_PRINTブロック
						{
							...
							pd.Flags = PD_RETURNDC | PD_PAGENUMS;	// pdのメンバとしてhDCを返す, ページ番号を指定する.
							pd.nMinPage = 1;	// 先頭ページは1.
							pd.nMaxPage = 4;	// 末尾ページは4.
							pd.nFromPage = 2;	// 開始ページは2.
							pd.nToPage = 3;		// 終了ページは3.
							...
							if (bRet){	// 印刷を押したらTRUE.
								...
								di.lpszDocName = _T("PRINTDLG_struct");	// ドキュメント名は"PRINTDLG_struct"とする.
								...
								if (iRetSD > 0){	// 印刷開始成功.
									...
									if (iRetSP > 0){	// ページ開始成功.
										...
										iRetSP = StartPage(pd.hDC);	// StartPageでページの開始.
										if (iRetSP > 0){	// ページ開始成功.
											TextOut(pd.hDC, 50, 100, _T("FGHIJ"), _tcslen(_T("FGHIJ")));	// TextOutでプリンタの(50, 100)の位置に"FGHIJ"を描画.
											EndPage(pd.hDC);	// EndPageでページ終了.
											/*
