...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetSubMenu");					// ウィンドウクラス名は"GetSubMenu".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetSubMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetSubMenu"), _T("GetSubMenu"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"GetSubMenu"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetSubMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_1:
						...
						// ID_ITEM_1_1ブロック
						{
							...
							// Item1を削除.
							HMENU hPopup1 = GetSubMenu(hMenu, 0);	// GetSubMenuで0番目のサブメニューハンドルhPopup1を取得.
							RemoveMenu(hMenu, 0, MF_BYPOSITION);	// RemoveMenuでhMenuの0番目のItem1を削除.(hPopup1は破棄されない.)
							BOOL bRet = DestroyMenu(hPopup1);	// DestroyMenuでhPopup1を破棄.
							if (bRet){	// bRetがTRUE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup1) success!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup1) success!"と表示.
							}
							else{	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup1) failed!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup1) failed!"と表示.
							}
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// Item2を削除.
							HMENU hPopup2 = GetSubMenu(hMenu, 1);	// GetSubMenuで1番目のサブメニューハンドルhPopup2を取得.
							RemoveMenu(hMenu, 1, MF_BYPOSITION);	// RemoveMenuでhMenuの1番目のItem2を削除.(hPopup2は破棄されない.)
							BOOL bRet = DestroyMenu(hPopup2);	// DestroyMenuでhPopup2を破棄.
							if (bRet){	// bRetがTRUE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup2) success!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup2) success!"と表示.
							}
							else{	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup2) failed!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup2) failed!"と表示.
							}
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// Item3を削除.
							HMENU hPopup3 = GetSubMenu(hMenu, 2);	// GetSubMenuで2番目のサブメニューハンドルhPopup3を取得.
							RemoveMenu(hMenu, 2, MF_BYPOSITION);	// RemoveMenuでhMenuの2番目のItem3を削除.(hPopup3は破棄されない.)
							BOOL bRet = DestroyMenu(hPopup3);	// DestroyMenuでhPopup3を破棄.
							if (bRet){	// bRetがTRUE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup3) success!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup3) success!"と表示.
							}
							else{	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopup3) failed!"), _T("GetSubMenu"), MB_OK);	// "DestroyMenu(hPopup3) failed!"と表示.
							}
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							TCHAR tszText[32] = _T("Item2-1 Clicked!");
							...
							GetMenuItemInfo(hPopupMenu2, 0, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.dwTypeData = tszText;
							SetMenuItemInfo(hPopupMenu2, 0, TRUE, &miiMenuItemInfo);
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							// メニュー項目名の設定.
							MENUITEMINFO miiMenuItemInfo = {0};
							TCHAR tszText[32] = _T("Item2-2 Clicked!");
							miiMenuItemInfo.cbSize = sizeof(miiMenuItemInfo);
							miiMenuItemInfo.fMask = MIIM_TYPE;
							miiMenuItemInfo.fType = MIIM_STRING;
							miiMenuItemInfo.dwTypeData = NULL;
							GetMenuItemInfo(hPopupMenu2, 1, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.dwTypeData = tszText;
							SetMenuItemInfo(hPopupMenu2, 1, TRUE, &miiMenuItemInfo);
					...
					case ID_ITEM_2_3:
						...
						// ID_ITEM_2_3ブロック
						{
							...
							// メニュー項目名の設定.
							MENUITEMINFO miiMenuItemInfo = {0};
							TCHAR tszText[32] = _T("Item2-3 Clicked!");
							miiMenuItemInfo.cbSize = sizeof(miiMenuItemInfo);
							miiMenuItemInfo.fMask = MIIM_TYPE;
							miiMenuItemInfo.fType = MIIM_STRING;
							miiMenuItemInfo.dwTypeData = NULL;
							GetMenuItemInfo(hPopupMenu2, 2, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.dwTypeData = tszText;
							SetMenuItemInfo(hPopupMenu2, 2, TRUE, &miiMenuItemInfo);
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							MENUITEMINFO miiMenuItemInfo = {0};
							miiMenuItemInfo.cbSize = sizeof(miiMenuItemInfo);
							miiMenuItemInfo.fMask = MIIM_STATE;
							GetMenuItemInfo(hPopupMenu3, 0, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.fState = MFS_GRAYED;
							SetMenuItemInfo(hPopupMenu3, 0, TRUE, &miiMenuItemInfo);
					...
					case ID_ITEM_3_2:
						...
						// ID_ITEM_3_2ブロック
						{
							...
							miiMenuItemInfo.cbSize = sizeof(miiMenuItemInfo);
							miiMenuItemInfo.fMask = MIIM_STATE;
							GetMenuItemInfo(hPopupMenu3, 1, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.fState = MFS_GRAYED;
							SetMenuItemInfo(hPopupMenu3, 1, TRUE, &miiMenuItemInfo);
					...
					case ID_ITEM_3_3:
						...
						// ID_ITEM_3_3ブロック
						{
							...
							miiMenuItemInfo.cbSize = sizeof(miiMenuItemInfo);
							miiMenuItemInfo.fMask = MIIM_STATE;
							GetMenuItemInfo(hPopupMenu3, 2, TRUE, &miiMenuItemInfo);
							miiMenuItemInfo.fState = MFS_GRAYED;
							SetMenuItemInfo(hPopupMenu3, 2, TRUE, &miiMenuItemInfo);
