...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_DragEnter");				// ウィンドウクラス名は"ImageList_DragEnter".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ってきたらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ImageList_DragEnter"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_DragEnter"), _T("ImageList_DragEnter"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ImageList_DragEnter"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ImageList_DragEnter"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
		...
		case WM_CREATE:		// ウィンドウの作成が開始されたとき.(uMsgがWM_CREATEの場合.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_DragEnter"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
					return -1;	// ウィンドウ作成失敗.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_DragEnter"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押されたとき.(uMsgがWM_LBUTTONDOWNの場合.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				if (ImageList_BeginDrag(hImageList, 0, 16, 16)){	// ImageList_BeginDragでhImageListの0番目の画像をドラッグ開始. ホットスポットは画像中央(16, 16).
					if (ImageList_DragEnter(hwnd, 50, 50)){	// ImageList_DragEnterでhwndをロックしてドラッグ画像を左上(50, 50)の位置に表示.
