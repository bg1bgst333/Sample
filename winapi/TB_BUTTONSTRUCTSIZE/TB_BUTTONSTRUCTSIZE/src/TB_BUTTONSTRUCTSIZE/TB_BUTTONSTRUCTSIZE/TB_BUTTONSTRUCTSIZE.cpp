...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("TB_BUTTONSTRUCTSIZE");					// ウィンドウクラス名"TB_BUTTONSTRUCTSIZE".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("TB_BUTTONSTRUCTSIZE"), _T("TB_BUTTONSTRUCTSIZE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
	...
	switch (uMsg){	// uMsgごとに処理振り分け.
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				tbb[1].iString = iOne;
				BOOL b = SendMessage(hToolBar, TB_ADDBUTTONS, (WPARAM)2, (LPARAM)(LPTBBUTTON)&tbb);
				TBADDBITMAP tbab2;		// TBADDBITMAP構造体tbab2.
				tbab2.hInst = HINST_COMMCTRL;
				tbab2.nID = IDB_STD_SMALL_COLOR;
				stdid = SendMessage(hToolBar, TB_ADDBITMAP, (WPARAM)1, (LPARAM)&tbab2);
				tbb[2].iBitmap = stdid + STD_FILENEW;	// なぜかこう.
				stdid++;
				int iNew = SendMessage(hToolBar, TB_ADDSTRING, 0, (LPARAM)_T("New"));
				tbb[2].iString = iNew;
				SendMessage(hToolBar, TB_ADDBUTTONS, (WPARAM)1, (LPARAM)(LPTBBUTTON)&tbb[2]);
				TBADDBITMAP tbab3;		// TBADDBITMAP構造体tbab3.
				tbab3.hInst =  HINST_COMMCTRL;
				tbab3.nID = IDB_VIEW_SMALL_COLOR;
				stdid = SendMessage(hToolBar, TB_ADDBITMAP, (WPARAM)1, (LPARAM)&tbab3);
				tbb[3].iBitmap = stdid + VIEW_PARENTFOLDER;	// なぜかこう.
				stdid++;
				int iParent = SendMessage(hToolBar, TB_ADDSTRING, 0, (LPARAM)_T("Parent"));
				tbb[3].iString = iParent;
				SendMessage(hToolBar, TB_ADDBUTTONS, (WPARAM)1, (LPARAM)(LPTBBUTTON)&tbb[3]);
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case IDM_ZERO:
						...
						// IDM_ZEROブロック
						{
							...
							// "Zero"と表示.
							MessageBox(hwnd, _T("Zero"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case IDM_ONE:
						...
						// IDM_ONEブロック
						{
							...
							// "One"と表示.
							MessageBox(hwnd, _T("One"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case IDM_TWO:
						...
						// IDM_TWOブロック
						{
							...
							// "Two"と表示.
							MessageBox(hwnd, _T("Two"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case IDM_THREE:
						...
						// IDM_THREEブロック
						{
							...
							// "Three"と表示.
							MessageBox(hwnd, _T("Three"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case ID_FILE_NEW:
						...
						// ID_FILE_NEWブロック
						{
							...
							// "ID_FILE_NEW"と表示.
							MessageBox(hwnd, _T("ID_FILE_NEW"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case ID_FOLDER_PARENT:
						...
						// ID_FOLDER_PARENTブロック
						{
							...
							MessageBox(hwnd, _T("ID_FOLDER_PARENT"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
					...
					case ID_FILE_OPEN:
						...
						// ID_FILE_OPENブロック
						{
							...
							// "ID_FILE_OPEN"と表示.
							MessageBox(hwnd, _T("ID_FILE_OPEN"), _T("TB_BUTTONSTRUCTSIZE"), MB_OK);
