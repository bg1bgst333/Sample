...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WC_EDIT");							// ウィンドウクラス名は"WC_EDIT".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("WC_EDIT"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("WC_EDIT"), tszWindowName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WC_EDIT"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("WC_EDIT"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				hStatic = CreateWindow(WC_STATIC, _T("StaticText1"), WS_CHILD | WS_VISIBLE, 50, 200, 100, 30, hwnd, (HMENU)ID_STATIC1, lpCS->hInstance, NULL);	// CreateWindowのWC_STATICでスタティックコントロール(テキスト)"StaticText1"を作成.
				...
				// エディットコントロールの作成.
				hEdit = CreateWindow(WC_EDIT, _T("Edit1"), WS_CHILD | WS_BORDER | WS_VISIBLE, 50, 350, 150, 30, hwnd, (HMENU)ID_EDIT1, lpCS->hInstance, NULL);	// CreateWindowのWC_EDITでエディットコントロール"Edit1"を作成.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目, またはコントロールのIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-2"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-2"と表示.
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-3"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-3"と表示.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-1"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-1"と表示.
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-2"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-2"と表示.
					...
					case ID_ITEM_2_3:
						...
						// ID_ITEM_2_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-3"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-3"と表示.
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-1"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-1"と表示.
					...
					case ID_ITEM_3_2:
						...
						// ID_ITEM_3_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-2"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-2"と表示.
					...
					case ID_ITEM_3_3:
						...
						// ID_ITEM_3_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-3"), _T("WC_EDIT"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-3"と表示.
