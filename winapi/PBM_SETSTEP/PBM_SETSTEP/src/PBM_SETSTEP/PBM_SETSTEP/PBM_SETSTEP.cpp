...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("PBM_SETSTEP");						// ウィンドウクラス名は"PBM_SETSTEP".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("PBM_SETSTEP"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("PBM_SETSTEP"), _T("PBM_SETSTEP"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"PBM_SETSTEP"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("PBM_SETSTEP"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				// 変数の宣言
				HWND hProgress;	// プログレスバーのハンドルhProgress.

				// プログレスバーの現在位置をセット.
				hProgress = GetDlgItem(hwndDlg, IDC_PROGRESS1);	// GetDlgItemでIDC_PROGRESS1のハンドルを取得.
				SendMessage(hProgress, PBM_SETRANGE, 0, MAKELPARAM(0, 200));	// SendMessageのPBM_SETRANGEでプログレスバーの範囲をMAKELPARAMマクロを使って0～200にセット.
				SendMessage(hProgress, PBM_SETSTEP, (WPARAM)1, 0);	// SendMessageのPBM_SETSTEPでプログレスバーの1ステップで進む数を1に設定.
				SendMessage(hProgress, PBM_SETPOS, (WPARAM)0, 0);	// SendMessageのPBM_SETPOSでプログレスバーの位置を0にセット.
		...
		case WM_CLOSE:	// ダイアログが閉じられた時.(uMsgがWM_CLOSEの時.)
			...
			// WM_CLOSEブロック
			{
				...
				// タイマーの終了.
				KillTimer(hwndDlg, ID_TIMER1);	// KillTimerでタイマーを破棄.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON1:
						...
						// ID_BUTTON1ブロック
						{
							...
							// プログレスバーを進捗させるタイマーの開始.
							SetTimer(hwndDlg, ID_TIMER1, 50, NULL);	// SetTimerで50ミリ秒間隔のタイマーをセット.
		...
		case WM_TIMER:	// タイマーイベントが発生した時.(uMsgがWM_TIMERの時.)
			...
			// WM_TIMERブロック
			{
				...
				// プログレスバーを進める.
				hProgress = GetDlgItem(hwndDlg, IDC_PROGRESS1);	// GetDlgItemでIDC_PROGRESS1のハンドルを取得.				
				SendMessage(hProgress, PBM_STEPIT, 0, 0);	// SendMessageのPBM_STEPITでプログレスバーを1ステップ進める.
