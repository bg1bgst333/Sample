...
// ウィンドウメッセージの定義
#define WM_SHNOTIFY 0x0401	// WM_SHNOTIFYを定義.
...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("SHCNE_MEDIAREMOVED");			// ウィンドウクラス名は"SHCNE_MEDIAREMOVED".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("SHCNE_MEDIAREMOVED"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("SHCNE_MEDIAREMOVED"), _T("SHCNE_MEDIAREMOVED"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"SHCNE_MEDIAREMOVED"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("SHCNE_MEDIAREMOVED"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	static std::map<HDEVNOTIFY, HANDLE> mapNotifyDirectoryMap;	// 通知ハンドルをキー, ディレクトリハンドルを値とするstd::map.
	static unsigned long ulNotificationId;	// 通知IDを保持するulNotificationId.
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				// シェル通知登録
				SHChangeNotifyEntry shcne = {0};	// SHChangeNotifyEntry構造体shcneを{0}で初期化.
				shcne.pidl = NULL;
				shcne.fRecursive = true;
				ulNotificationId = SHChangeNotifyRegister(hwnd, SHCNF_TYPE | SHCNF_IDLIST, SHCNE_MEDIAINSERTED | SHCNE_MEDIAREMOVED, WM_SHNOTIFY, 1, &shcne);	// SHChangeNotifyRegisterで登録.

				// ulNotificationIdをテキスト変換.
				TCHAR tszId[1024] = {0};	// TCHAR文字配列tszId(長さ1024)を{0}で初期化.
				_stprintf(tszId, _T("ulNotificationId = %lu\n"), ulNotificationId);	// _stprintfでulNotificationIdをtszIdに変換.																																										// tszDBHを表示.
				OutputDebugString(tszId);	// OutputDebugStringでtszIdを表示.
		...
		case WM_DESTROY:	// ウィンドウが破棄された時.(uMsgがWM_DESTROYの時.)
			...
			// WM_DESTROYブロック
			{
				...
				// シェル通知解除
				if (ulNotificationId != 0){
					BOOL bRet = SHChangeNotifyDeregister(ulNotificationId);	// SHChangeNotifyDeregisterで解除.
				}
				...
				switch (wParam){	// wParamごとに振り分け.
								...
								case DBT_DEVTYP_VOLUME:
									...
									// DBT_DEVTYP_VOLUMEブロック
									{
										...
										if (hDirectory != INVALID_HANDLE_VALUE){	// 成功.
											...
											mapNotifyDirectoryMap[hDevNotify] = hDirectory;	// キーhDevNotifyに値hDirectoryをセット.
										}
								...
								case DBT_DEVTYP_HANDLE:
									...
									// DBT_DEVTYP_HANDLEブロック
									{
										...
										if (pDBHandle->dbch_handle != NULL){	// ディレクトリ開いてたら.
											...
											OutputDebugString(_T("CloseHandle(pDBHandle->dbch_handle);\n"));
											pDBHandle->dbch_handle = NULL;	// NULLをセット.
											mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify] = NULL;	// mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify]をNULL.
										}
										...
										if (mapNotifyDriveMap.find(pDBHandle->dbch_hdevnotify) != mapNotifyDriveMap.end()){	// 存在する.
											...
											if (hDirectory != INVALID_HANDLE_VALUE){	// 成功.
												...
												if (bRet){
													...
													mapNotifyDirectoryMap.erase(pDBHandle->dbch_hdevnotify);	// mapNotifyDirectoryMapからも削除.
													// DEV_BROADCAST_HANDLEの準備.
													DEV_BROADCAST_HANDLE dbhandle = {0};	// DEV_BROADCAST_HANDLE構造体dbhandleを{0}で初期化.
													dbhandle.dbch_size = sizeof(DEV_BROADCAST_HANDLE);	// サイズはsizeof(DEV_BROADCAST_HANDLE).
													dbhandle.dbch_devicetype = DBT_DEVTYP_HANDLE;	// タイプはDBT_DEVTYP_HANDLE.
													...
													mapNotifyDirectoryMap[hDevNotify] = hDirectory;	// mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify]にhDirectoryをセット.
													OutputDebugString(_T("CreateFile(hDirectory);\n"));
												}
											}
										}
					...
					case DBT_DEVICEREMOVEPENDING:
						...
						// DBT_DEVICEREMOVEPENDINGブロック
						{
							...
							// "DBT_DEVICEREMOVEPENDING"と表示.
							OutputDebugString(_T("DBT_DEVICEREMOVEPENDING\n"));	// OutputDebugStringで"DBT_DEVICEREMOVEPENDING"と表示.
							...
							switch (pDBH->dbch_devicetype){
								...
								case DBT_DEVTYP_HANDLE:
									...
									// DBT_DEVTYP_HANDLEブロック
									{
										...
										// mapNotifyDirectoryMapでキーpDBHandle->dbch_hdevnotifyとなる要素の値を閉じる.
										if (mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify] != NULL){	// NULLでなければ.
											CloseHandle(mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify]);	// 閉じる.
											mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify] = NULL;	// mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify]をNULL.
										}
										else{
											OutputDebugString(_T("Already CloseHandle(mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify]);\n"));
										}
								...
								case DBT_DEVTYP_VOLUME:
									...
									// DBT_DEVTYP_VOLUMEブロック
									{
										...
										if (hUnregistered != NULL){	// 解除済み通知ハンドルがあったら.
											...
											mapNotifyDirectoryMap.erase(hUnregistered);	// キーhUnregistered要素を削除. 
										}
								...
								case DBT_DEVTYP_HANDLE:
									...
									// DBT_DEVTYP_HANDLEブロック
									{
										...
										if (bRet){
											...
											mapNotifyDirectoryMap.erase(pDBHandle->dbch_hdevnotify);	// キーpDBHandle->dbch_hdevnotify要素を削除.
											OutputDebugString(_T("mapNotifyDirectoryMap.erase(pDBHandle->dbch_hdevnotify)\n"));
										}
					...
					case DBT_CUSTOMEVENT:
						...
						{	
							...
							// "DBT_CUSTOMEVENT"と表示.
							OutputDebugString(_T("DBT_CUSTOMEVENT\n"));	// OutputDebugStringで"DBT_CUSTOMEVENT"と表示.
							...
							switch (pDBH->dbch_devicetype){
								...
								case DBT_DEVTYP_HANDLE:
									...
									// DBT_DEVTYP_HANDLEブロック
									{
										...
										// ここでディレクトリハンドルを閉じる.
										if (mapNotifyDirectoryMap[pDBHandle->dbch_hdevnotify] != NULL){	// ディレクトリ開いてたら.
											CloseHandle(pDBHandle->dbch_handle);	// 本来はここで閉じる.
											mapNotifyDirectoryMap.erase(pDBHandle->dbch_hdevnotify);	// キーpDBHandle->dbch_hdevnotify要素を削除.
											OutputDebugString(_T("CloseHandle(pDBHandle->dbch_handle);\n"));
											// 通知ハンドルの解除.
		...
		case WM_SHNOTIFY:
			...
			// WM_SHNOTIFYブロック
			{
				...
				// "WM_SHNOTIFY"と表示.
				OutputDebugString(_T("WM_SHNOTIFY\n"));	// OutputDebugStringで"WM_SHNOTIFY"と表示.

				// イベントに対する処理.
				switch (lParam){	// lParamごとに振り分け.

					// メディアが挿入された時.
					case SHCNE_MEDIAINSERTED:

						// SHCNE_MEDIAINSERTEDブロック
						{

							// "SHCNE_MEDIAINSERTED"と表示.
							OutputDebugString(_T("SHCNE_MEDIAINSERTED\n"));	// OutputDebugStringで"SHCNE_MEDIAINSERTED"と表示.
					...
					case SHCNE_MEDIAREMOVED:
						...
						// SHCNE_MEDIAREMOVEDブロック
						{

							// "SHCNE_MEDIAREMOVED"と表示.
							OutputDebugString(_T("SHCNE_MEDIAREMOVED\n"));	// OutputDebugStringで"SHCNE_MEDIAREMOVED"と表示.
