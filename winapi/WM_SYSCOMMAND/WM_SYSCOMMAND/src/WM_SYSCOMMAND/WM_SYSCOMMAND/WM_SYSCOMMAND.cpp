...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_SYSCOMMAND");					// ウィンドウクラス名は"WM_SYSCOMMAND".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("WM_SYSCOMMAND"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_SYSCOMMAND"), _T("WM_SYSCOMMAND"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WM_SYSCOMMAND"ウィンドウクラスのウィンドウを作成.(9番目にNULLを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("WM_SYSCOMMAND"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		// システムメニュー項目が選択された時.
		case WM_SYSCOMMAND:	// システムメニュー項目が選択された時.(uMsgがWM_SYSCOMMANDの時.)
			...
			// WM_SYSCOMMANDブロック
			{
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					// 取得したIDごとに処理を分岐.
					// SystemMenuItem1が選択された時.
					case IDM_SYSTEM_ITEM_101:

						// IDM_SYSTEM_ITEM_101ブロック
						{

							// "SystemMenuItem1"と表示.
							MessageBox(hwnd, _T("SystemMenuItem1"), _T("WM_SYSCOMMAND"), MB_OK);	// MessageBoxで"SystemMenuItem1"と表示.
