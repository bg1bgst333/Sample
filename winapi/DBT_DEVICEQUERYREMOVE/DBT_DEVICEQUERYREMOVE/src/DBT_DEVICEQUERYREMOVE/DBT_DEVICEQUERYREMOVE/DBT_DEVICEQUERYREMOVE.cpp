...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("DBT_DEVICEQUERYREMOVE");			// ウィンドウクラス名は"DBT_DEVICEQUERYREMOVE".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("DBT_DEVICEQUERYREMOVE"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("DBT_DEVICEQUERYREMOVE"), _T("DBT_DEVICEQUERYREMOVE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"DBT_DEVICEQUERYREMOVE"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("DBT_DEVICEQUERYREMOVE"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	// スタティック変数の初期化.
	static HDEVNOTIFY hDevNotify = NULL;	// 通知ハンドルhDevNotifyをNULLで初期化.
	static HANDLE hDirectory = NULL;	// ディレクトリハンドルhDirectoryをNULLで初期化.
							...
							switch (pDBH->dbch_devicetype){
								...
								case DBT_DEVTYP_VOLUME:
									...
									// DBT_DEVTYP_VOLUMEブロック
									{
										...
										// ドライブのルートディレクトリを開く.
										hDirectory = CreateFile(tszLetter, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);	// CreateFileでtszLetterを開く.(FILE_FLAG_BACKUP_SEMANTICSでディレクトリが開ける.)
										if (hDirectory != INVALID_HANDLE_VALUE){	// 成功.
											...
											if (hDevNotify == NULL){	// NULLなら.
												// デバイス通知登録.
												hDevNotify = RegisterDeviceNotification(hwnd, &dbhandle, DEVICE_NOTIFY_WINDOW_HANDLE);	// RegisterDeviceNotificationでdbhandleを登録し, hDevNotifyを得る.
							...
							switch (pDBH->dbch_devicetype){
								...
								case DBT_DEVTYP_HANDLE:
									...
									// DBT_DEVTYP_HANDLEブロック
									{
										...
										// ここでディレクトリハンドルを閉じる.
										if (hDirectory != NULL){	// ディレクトリ開いてたら.
											CloseHandle(hDirectory);	// 本来はここで閉じる.
											hDirectory = NULL;	// NULLをセット.
										}
							...
							switch (pDBH->dbch_devicetype){
								...
								case DBT_DEVTYP_VOLUME:
									...
									// DBT_DEVTYP_VOLUMEブロック
									{
										...
										// 通知ハンドルの解除.
										if (hDevNotify != NULL){	// NULLなら.
											BOOL bRet = UnregisterDeviceNotification(hDevNotify);	// UnregisterDeviceNotificationで解除.
											if (bRet){
												hDevNotify = NULL;
											}
										}
