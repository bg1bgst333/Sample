						...
						if (dwRet == CR_SUCCESS){	// 成功.
							if ((ulStatus & DN_DISABLEABLE) && (ulStatus & DN_REMOVABLE)){	// 無効化可能かつ取り外し可能.
								...
								if (dwRet == CR_SUCCESS){	// 成功.
									...
									// デバイスIDの取得.
									TCHAR *ptszDevID = new TCHAR[ulBufLen + 1];	// サイズがulBufLen + 1なTCHAR動的配列をnewで確保し, ポインタをptszDevIDに格納.
									dwRet = CM_Get_Device_ID(spdd.DevInst, ptszDevID, ulBufLen + 1, 0);	// CM_Get_Device_IDでデバイスIDを取得.
									if (dwRet == CR_SUCCESS){	// 成功.
