...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("SetDIBits");						// ウィンドウクラス名は"SetDIBits".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("SetDIBits"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("SetDIBits"), _T("SetDIBits"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"SetDIBits"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("SetDIBits"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理をするように定義したウィンドウプロシージャ.
	...
	static HBITMAP hDDB;			// DIBからDDBへの変換で得られたビットマップハンドルを格納するHBITMAP型スタティック変数hDDB.
	static LPBYTE lpBitsPixel;		// ピクセルビット列lpBitsPixel.
	static HDC hBackMemDC;			// hBackBitmap用のデバイスコンテキストhBackMemDC.
	static HDC hDDBMemDC;			// hDDB用のデバイスコンテキストhDDBMemDC.
	static HBITMAP hOldBackBitmap;	// 選択前のバックバッファビットマップhOldBackBitmap.
	static HBITMAP hOldDDB;			// 選択前のDDB側のビットマップhOldDDB.
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				// DIBからDDBを作成.
				hDDB = CreateDIBitmap(hDC, &bi.bmiHeader, CBM_INIT, lpBitsPixel, &bi, DIB_RGB_COLORS);	// CreateDIBitmapでピクセル列からDDBを作成.
				...
				// DDBの選択.
				hOldDDB = (HBITMAP)SelectObject(hDDBMemDC, hDDB);					// SelectObjectでhDDBを選択.
		...
		case WM_DESTROY:	// ウィンドウが破棄された時.(uMsgがWM_DESTROYの時.)
			...
			// WM_DESTROYブロック
			{
				...
				// 元に戻す.
				SelectObject(hDDBMemDC, hOldDDB);			// SelectObjectでhOldDDBに戻す.
				...
				// DDBのビットマップハンドル削除.
				if (hDDB){	// hDDBがNULLでない時.
					DeleteObject(hDDB);	// DeleteObjectでhDDBを削除.
					hDDB = NULL;	// NULLを代入.
				}
		...
		case WM_PAINT:		// 画面の描画を要求された時.(uMsgがWM_PAINTの時.)
			...
			// WM_PAINTブロック
			{
				...
				// DIBをDDBに変換したものを描画.
				BitBlt(hBackMemDC, 0, 0, bi.bmiHeader.biWidth, bi.bmiHeader.biHeight, hDDBMemDC, 0, 0, SRCCOPY);	// BitBltでhDDBMemDCをhBackMemDCに描画.
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押された時.(uMsgがWM_LBUTTONDOWNの時.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				if (x >= 5 && x <= 634 && y >= 5 && y <= 474){
					...
					// (x - 5, y - 5)-(x + 5, y + 5)な矩形になるようにピクセル列をセット.
					for (j = -5; j <= 5; j++){
						for (i = -5; i <= 5; i++){
							lpBitsPixel[(640 * 3 + (640 * 3) % 4) * (480 - (y + j + 1)) + (x + i) * 3] = 0x00;		// 青
							lpBitsPixel[(640 * 3 + (640 * 3) % 4) * (480 - (y + j + 1)) + (x + i) * 3 + 1] = 0x00;	// 緑
							lpBitsPixel[(640 * 3 + (640 * 3) % 4) * (480 - (y + j + 1)) + (x + i) * 3 + 2] = 0x00;	// 赤
						}
					}

					// 描画
					hDC = GetDC(hwnd);	// GetDCでデバイスコンテキスト取得.
					SetDIBits(hDC, hDDB, 0, 480, lpBitsPixel, &bi, DIB_RGB_COLORS);	// SetDIBitsでピクセル列をhDDBにセット.
					ReleaseDC(hwnd, hDC);	// ReleaseDCでデバイスコンテキスト解放.
					SelectObject(hDDBMemDC, hDDB);	// SelectObjectでDDB用メモリデバイスコンテキストにhDBBをセット.

					// 画面更新.
					InvalidateRect(hwnd, NULL, TRUE);	// InvalidateRectで無効領域として画面更新を促す.
