...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LVN_ODCACHEHINT");					// ウィンドウクラス名は"LVN_ODCACHEHINT".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("LVN_ODCACHEHINT"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("LVN_ODCACHEHINT"), _T("LVN_ODCACHEHINT"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"LVN_ODCACHEHINT"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("LVN_ODCACHEHINT"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case LVN_ODCACHEHINT:	// オーナーデータでキャッシュヒント.
							...
							// LVN_ODCACHEHINTブロック.
							{
								...
								// デバッグ出力
								OutputDebugString(_T("LVN_ODCACHEHINT\n"));	// OutputDebugStringで"LVN_ODCACHEHINT"を出力.
