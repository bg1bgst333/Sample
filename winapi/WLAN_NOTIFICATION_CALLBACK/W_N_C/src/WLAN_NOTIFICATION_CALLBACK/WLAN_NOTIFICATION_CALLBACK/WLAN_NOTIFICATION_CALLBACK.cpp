...
// グローバル変数の初期化
BOOL bWait = TRUE;	// 完了待ちフラグbWaitをTRUEで初期化.
...
static void WlanNotificationCallback(PWLAN_NOTIFICATION_DATA Arg1, PVOID Arg2){
	...
	if (Arg1->NotificationSource == WLAN_NOTIFICATION_SOURCE_ACM){	// 通知元がACMの時.
		...
		if (Arg1->NotificationCode == wlan_notification_acm_scan_complete){	// スキャン完了.
			_tprintf(_T("wlan_notification_acm_scan_complete\n"));	// "wlan_notification_acm_scan_complete"と出力.
			bWait = FALSE;	// FALSEにする.
		}
		else if (Arg1->NotificationCode == wlan_notification_acm_scan_fail){	// スキャン失敗.
			WLAN_CONNECTION_NOTIFICATION_DATA *pWlanConnNotifData = (WLAN_CONNECTION_NOTIFICATION_DATA *)Arg1->pData;	// 接続通知データを取り出す.
			if (pWlanConnNotifData->wlanReasonCode != ERROR_SUCCESS){	// 原因が何らかのエラー.
				_tprintf(_T("wlan_notification_acm_scan_fail\n"));	// "wlan_notification_acm_scan_fail"と出力.
				bWait = FALSE;	// FALSEにする.
			}
		}
	}
