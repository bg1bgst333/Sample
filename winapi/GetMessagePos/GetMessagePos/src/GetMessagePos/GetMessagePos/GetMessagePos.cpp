...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetMessagePos");						// ウィンドウクラス名は"GetMessagePos".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetMessagePos"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetMessagePos"), _T("GetMessagePos"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetMessagePos"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetMessagePos"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case NM_CLICK:	// クリック.
							...
							// NM_CLICKブロック.
							{
								...
								// 最後のメッセージの時のカーソル位置(DWORD)を出力.
								DWORD dwPos = GetMessagePos();	// GetMessagePosで最後のメッセージのカーソル位置を取得.
								TCHAR tszPos[64] = {0};
								_stprintf(tszPos, _T("NM_CLICK: dwPos = 0x%08x\n"), dwPos);
								OutputDebugString(tszPos);	// OutputDebugStringでtszPosを出力.
