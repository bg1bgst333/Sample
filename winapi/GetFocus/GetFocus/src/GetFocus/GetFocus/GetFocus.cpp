...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetFocus");						// ウィンドウクラス名は"GetFocus".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("GetFocus"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetFocus"), _T("GetFocus"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetFocus"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("GetFocus"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							MessageBox(hwnd, tszText, _T("GetFocus"), MB_OK);	// tszTextを表示.
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							MessageBox(hwnd, tszPoint, _T("GetFocus"), MB_OK);	// tszPointを表示.
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							MessageBox(hwnd, tszPos, _T("GetFocus"), MB_OK);	// tszPosを表示.
							...
							MessageBox(hwnd, tszResult, _T("GetFocus"), MB_OK);	// tszResultを表示.
					...
					case ID_ITEM_8_1:
						...
						// ID_ITEM_8_1ブロック
						{
							...
							// フォーカスを取得したウィンドウを判定.
							HWND hWnd = GetFocus();	// GetFocusでフォーカス取得したウィンドウハンドルhWndを取得.
							if (hWnd == hEdit1){	// hEdit1なら.
								MessageBox(hwnd, _T("Edit1"), _T("GetFocus"), MB_OK);	// "Edit1"と表示.
							}
							else if (hWnd == hEdit2){	// hEdit2なら.
								MessageBox(hwnd, _T("Edit2"), _T("GetFocus"), MB_OK);	// "Edit2"と表示.
							}

						}

						// 既定の処理へ向かう.
						break;	// breakで抜けて, 既定の処理(DefWindowProc)へ向..かう.
