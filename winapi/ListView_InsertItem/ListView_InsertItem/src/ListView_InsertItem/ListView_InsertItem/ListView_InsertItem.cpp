...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ListView_InsertItem");				// ウィンドウクラス名は"ListView_InsertItem".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ListView_InsertItem"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ListView_InsertItem"), _T("ListView_InsertItem"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ListView_InsertItem"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ListView_InsertItem"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON1:
						...
						// ID_BUTTON1ブロック
						{
							...
							HWND hList;				// リストビューのハンドルhList.
							LVITEM item = {0};		// 項目を格納するLVITEM構造体変数itemを0で初期化.
							...
							// LVITEM構造体変数itemに設定.
							item.mask = LVIF_TEXT;	// 表示テキストのみセット.
							item.iItem = 0;			// 一番上から挿入.
							item.pszText = tszName;	// 表示テキストはtszName.

							// itemの挿入.
							hList = GetDlgItem(hwndDlg, IDC_LIST1);	// GetDlgItemでIDC_LIST1のハンドル取得.
							ListView_InsertItem(hList, &item);		// ListView_InsertItemでhListにitemを挿入.
