...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("SetMapMode");					// ウィンドウクラス名は"SetMapMode".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("SetMapMode"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("SetMapMode"), _T("SetMapMode"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"SetMapMode"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("SetMapMode"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
							...
							if (hPrinterDC != NULL){	// hPrinterDCがNULLでなければ.
								...
								if (iRetSD > 0){	// 印刷開始成功.
									...
									if (iRetSP > 0){	// ページ開始成功.
										...
										TextOut(hPrinterDC, 10, -10, _T("ABCDE"), _tcslen(_T("ABCDE")));	// TextOutでプリンタの(10, -10)の位置に"ABCDE"を描画.(Y軸は上に向かって増加なので, マイナスにすると下に移動する.)
