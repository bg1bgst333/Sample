...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetSaveFileName");				// ウィンドウクラス名は"GetSaveFileName".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetSaveFileName"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetSaveFileName"), _T("GetSaveFileName"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetSaveFileName"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetSaveFileName"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_FILE_OPEN:
						...
						// ID_FILE_OPENブロック
						{
							...
							if (hFile == INVALID_HANDLE_VALUE){	// hFileがINVALID_HANDLE_VALUEなら.
								...
								// エラー処理
								MessageBox(NULL, _T("Error!\n"), _T("GetSaveFileName"), MB_ICONHAND);	// MessageBoxでエラーメッセージ.
								return 0;	// 処理したので戻り値として0を返す.
					...
					case ID_FILE_SAVEAS:
						...
						// ID_FILE_SAVEASブロック
						{
							...
							HANDLE hFile = NULL;	// ファイルハンドルhFileをNULLに初期化.
							HWND hEdit;	// エディットボックスのウィンドウハンドルhEdit.
							DWORD dwLen;	// DWORD型テキストの長さdwLen.
							LPBYTE lpBytes;	// エディットボックスの内容を格納するBYTE型メモリへのポインタlpBytes.
							...
							// エディットボックスのテキストの長さを週億.
							dwLen = GetWindowTextLengthA(hEdit);	// GetWindowTextLengthA(GetWindowTextLengthのASCII(ShiftJIS)版)で長さを取得.

							// ファイルの開く.
							hFile = CreateFile(tszFilePath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);	// CreateFileで選択したファイルを書込みモード(GENERIC_WRITE)開いて, 取得したハンドルをhFileに格納.
							if (hFile == INVALID_HANDLE_VALUE){	// hFileがINVALID_HANDLE_VALUEなら.
							...
							// メモリの確保.
							lpBytes = new BYTE[dwLen + 1];	// dwLen + 1メモリを確保.

							// エディットボックスのテキストを取得.
							GetWindowTextA(hEdit, (LPSTR)lpBytes, dwLen + 1);	// GetWindowTextA(GetWindowTextのASCII(ShiftJIS)版)でテキストを取得.

							// ファイルの書き込み.
							WriteFile(hFile, lpBytes, dwLen, &dwLen, NULL);	// WriteFileで書き込み.
