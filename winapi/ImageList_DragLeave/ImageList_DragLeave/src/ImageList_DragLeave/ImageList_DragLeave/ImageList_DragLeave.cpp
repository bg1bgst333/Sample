...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_DragLeave");				// ウィンドウクラス名を"ImageList_DragLeave".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ってきたらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ImageList_DragLeave"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_DragLeave"), _T("ImageList_DragLeave"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ImageList_DragLeave"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ImageList_DragLeave"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理ができるように定義したウィンドウプロシージャ.
	...
	static BOOL bImageHidden = FALSE;		// 禁止エリア進入によりドラッグ画像を隠しているかどうかを示すBOOL型スタティック変数bImageHidden.
	static RECT rcLimit;					// ドラッグ画像の移動を制限するエリア(枠として描画する領域)を表すRECT型スタティック変数rcLimit.
	static RECT rcHotspotLimit;			// ホットスポット(pt, 画像左上)の可動範囲を表すRECT型スタティック変数rcHotspotLimit. rcLimitの右辺・下辺を画像サイズ(32px)分だけ内側に縮めたもの.
	static RECT rcForbidden;				// ImageList_DragLeaveを起動する禁止エリアを表すRECT型スタティック変数rcForbidden. rcLimit内右側に配置.
	...
	switch (uMsg){	// switch-case文でuMsgの値をもとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始されたとき.(uMsgがWM_CREATEの場合.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_DragLeave"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
					return -1;	// ウィンドウ作成失敗.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_DragLeave"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
		...
		case WM_PAINT:		// 画面の描画が求められたとき.(uMsgがWM_PAINTの場合.)
			...
			// WM_PAINTブロック
			{
				...
				HBRUSH hBrushForbidden;	// 禁止エリア塗りつぶし用のHBRUSH型変数hBrushForbidden.
				...
				// 禁止エリアを塗りつぶす. ドラッグ画像が隠れている間(bImageHidden)は濃い赤, それ以外は薄い赤.
				hBrushForbidden = CreateSolidBrush(bImageHidden ? RGB(255, 0, 0) : RGB(255, 200, 200));	// CreateSolidBrushでbImageHiddenに応じた色のブラシを作成.
				FillRect(hDC, &rcForbidden, hBrushForbidden);	// FillRectでrcForbiddenを塗りつぶす.
		...
		case WM_MOUSEMOVE:	// マウスが移動したとき.(uMsgがWM_MOUSEMOVEの場合.)
			...
			// WM_MOUSEMOVEブロック
			{
				...
				// ドラッグ中なら, ドラッグ画像を制御エリア内に収めながら移動, かつ禁止エリアの出入りを監視.
				if (bDragging){
					...
					// ImageList_DragEnterはクライアント座標のままでよいが, ImageList_DragMoveはウィンドウ左上
					// (タイトルバー・枠含む)基準の座標が必要なため, 両方で使えるようptWinを用意しておく.
					POINT ptWin = pt;
					ClientToScreen(hwnd, &ptWin);	// ClientToScreenでクライアント座標からスクリーン座標に変換.

					RECT rcWindow;
					GetWindowRect(hwnd, &rcWindow);	// GetWindowRectでウィンドウ全体(タイトルバー・枠まで)のスクリーン座標での矩形を取得.
					ptWin.x -= rcWindow.left;			// ウィンドウの左上を原点とした座標に変換.
					ptWin.y -= rcWindow.top;

					// ドラッグ画像の矩形(ホットスポットだけでなく画像全体)が禁止エリア(rcForbidden)と
					// 少しでも重なっているかどうかを判定する(画像の右下側などが先に触れても反応させるため).
					if (IsImageOverlapForbidden(pt, &rcForbidden)){	// 禁止エリアと重なっている場合.

						if (!bImageHidden){	// まだ隠していない場合(禁止エリアに触れた瞬間).

							ImageList_DragLeave(hwnd);	// ImageList_DragLeaveでhwndのロックを解除し, ドラッグ画像を隠す.
							bImageHidden = TRUE;		// bImageHiddenをTRUEに設定.

							// 禁止エリアを塗り直す(濃い赤に切り替える)ため即座に再描画.
							InvalidateRect(hwnd, &rcForbidden, FALSE);
							UpdateWindow(hwnd);

						}
						// 既に隠れている場合は, 画像は非表示のままなので何もしない(ImageList_DragMoveは呼ばない).

					}
					else{	// 禁止エリア外の場合.

						if (bImageHidden){	// 隠れていた場合(禁止エリアから出た瞬間).

							ImageList_DragEnter(hwnd, ptWin.x, ptWin.y);	// ImageList_DragEnterでhwndを再度ロックし, ドラッグ画像を現在位置(ptWin)に再表示.
							bImageHidden = FALSE;		// bImageHiddenをFALSEに設定.

							// 禁止エリアを塗り直す(薄い赤に戻す)ため即座に再描画.
							InvalidateRect(hwnd, &rcForbidden, FALSE);
							UpdateWindow(hwnd);
							...
							ImageList_DragMove(ptWin.x, ptWin.y);	// ImageList_DragMoveでドラッグ画像を(ptWin.x, ptWin.y)(ウィンドウ座標)の位置に移動.
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押されたとき.(uMsgがWM_LBUTTONDOWNの場合.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				// 禁止エリア(rcForbidden)に重なった状態でのクリックは, ドラッグ自体は開始し,
				// その場で非表示+赤の状態にする(下記IsImageOverlapForbiddenでの分岐で扱う).
				if (PtInRect(&rcLimit, pt)){

					// クリック位置(画像の左上になる)が枠の右端・下端に近く, 画像がはみ出す場合だけクランプする.
					// (rcHotspotLimitの左端・上端はrcLimitと同じなので, ここでは右端・下端だけ見ればよい.)
					if (pt.x >= rcHotspotLimit.right){
						pt.x = rcHotspotLimit.right - 1;
					}
					if (pt.y >= rcHotspotLimit.bottom){
					...
					if (ImageList_BeginDrag(hImageList, 0, 0, 0)){	// ImageList_BeginDragでhImageListの0番目の画像をドラッグ開始. ホットスポットは画像左上(0, 0).

						// ImageList_DragEnterは, hwndの左上角(タイトルバー・枠を含む)基準にした座標を要求する.
						POINT ptWin = pt;
						ClientToScreen(hwnd, &ptWin);	// ClientToScreenでクライアント座標からスクリーン座標に変換.
						...
						if (IsImageOverlapForbidden(pt, &rcForbidden)){	// クリックした時点で既に禁止エリアと重なっている場合.

							// ImageList_DragEnterを呼ばず, 最初から非表示状態でドラッグを開始する.
							SetCapture(hwnd);		// SetCaptureでマウスキャプチャを設定.
							bDragging = TRUE;		// bDraggingをTRUEに設定.
							bImageHidden = TRUE;	// bImageHiddenをTRUEに設定(最初から非表示).

							// 禁止エリアを赤く塗り直すため即座に再描画.
							InvalidateRect(hwnd, &rcForbidden, FALSE);
							UpdateWindow(hwnd);

						}
						else if (ImageList_DragEnter(hwnd, ptWin.x, ptWin.y)){	// ImageList_DragEnterでhwndをロックしてドラッグ画像をクリック位置(ptWin)(ウィンドウ座標)に表示.
							SetCapture(hwnd);		// SetCaptureでマウスキャプチャを設定.
							bDragging = TRUE;		// bDraggingをTRUEに設定.
							bImageHidden = FALSE;	// bImageHiddenをFALSEに設定.
						}
		...
		case WM_LBUTTONUP:	// マウスの左ボタンが離されたとき.(uMsgがWM_LBUTTONUPの場合.)
			...
			// WM_LBUTTONUPブロック
			{
				...
				// ドラッグ中なら終える.
				if (bDragging){
					if (!bImageHidden){	// ドラッグ画像がまだ表示されている場合だけロック解除する.
