...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_GetImageCount");			// ウィンドウクラス名"ImageList_GetImageCount".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_GetImageCount"), _T("ImageList_GetImageCount"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap1 == NULL){	// hBitmap1がNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_GetImageCount"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
				...
				// イメージリストの作成
				hImageList = ImageList_Create(32, 32, ILC_COLOR24 | ILC_MASK, 1, 1);	// ImageList_Createでイメージリストを作成.(ILC_MASKを付けないとマスクが有効にならない.)
				if (hImageList == NULL){	// hImageListがNULLなら失敗.

					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_GetImageCount"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
				...
				ImageList_AddMasked(hImageList, hBitmap2, RGB(255, 0, 255));	// ImageList_AddMaskedでhImageListにマスクが(255, 0, 255)となるマスクを含むhBitmap2を追加.
