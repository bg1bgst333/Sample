...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ReplaceText");						// ウィンドウクラス名"ReplaceText".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("ReplaceText"), _T("ReplaceText"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	if (uMsg == uMessage){
		if (fr.Flags & FR_FINDNEXT){	// 次を検索を押された時.
			// 検索文字列を表示.
			MessageBox(hwnd, tszFind, _T("ReplaceText"), MB_OK);
		}
		if (fr.Flags & FR_REPLACE){	// 置換して次にを押された時.
			// 置換文字列を表示.
			MessageBox(hwnd, tszReplace, _T("ReplaceText"), MB_OK);
		}
		return 0;	// 0を返す.
	}
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_REPLACE_DIALOG:
						...
						// ID_REPLACE_DIALOGブロック
						{
							...
							// 置換ダイアログの表示.
							fr.lpstrReplaceWith = tszReplace;
							fr.wReplaceWithLen = 80;
							ReplaceText(&fr);	// ReplaceTextで置換ダイアログを表示.
