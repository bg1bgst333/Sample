	...
	switch (uMsg){	// uMsgごとに処理振り分け.
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				cc.Flags = CC_FULLOPEN | CC_RGBINIT | CC_ENABLEHOOK | CC_ENABLETEMPLATE;	// CC_ENABLETEMPLATEを追加.
				cc.lpfnHook = CCHookProc;
				cc.lpTemplateName = MAKEINTRESOURCE(IDD_CUSTOM_COLOR_DIALOG);	// ここに差し替えるダイアログリソースIDを指定.
...
UINT CALLBACK CCHookProc(HWND hDlg, UINT uiMsg, WPARAM wp, LPARAM lp){
	switch (uiMsg){
		case WM_INITDIALOG:
			SetWindowText(hDlg, _T("CHOOSECOLOR_struct!!!"));
			return TRUE;
	}
	return FALSE;
}