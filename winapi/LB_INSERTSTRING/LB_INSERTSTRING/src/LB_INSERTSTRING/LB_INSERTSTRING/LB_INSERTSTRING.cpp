...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LB_INSERTSTRING");					// ウィンドウクラス名は"LB_INSERTSTRING".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("LB_INSERTSTRING"), tszWindowName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"LB_INSERTSTRING"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-2"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-2"と表示.
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item1-3"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item1-3"と表示.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-1"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-1"と表示.
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-2"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-2"と表示.
					...
					case ID_ITEM_2_3:
						...
						// ID_ITEM_2_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item2-3"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item2-3"と表示.
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-1"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-1"と表示.
					...
					case ID_ITEM_3_2:
						...
						// ID_ITEM_3_2ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-2"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-2"と表示.
					...
					case ID_ITEM_3_3:
						...
						// ID_ITEM_3_3ブロック
						{
							...
							// メッセージボックスを表示.
							MessageBox(NULL, _T("Item3-3"), _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Item3-3"と表示.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON1:
						...
						// ID_BUTTON1ブロック
						{
							...
							// 変数・配列の宣言
							HWND hEdit;	// ID_EDIT1のウィンドウハンドルhEdit.
							HWND hList;	// ID_LIST1のウィンドウハンドルhList.
							TCHAR tszBuf[256];	// ID_EDIT1のテキスト内容を格納するバッファtszBuf.(長さ256)

							// エディットボックスID_EDIT1のテキスト内容を取得.
							hEdit = GetDlgItem(hwndDlg, ID_EDIT1);	// GetDlgItemでID_EDIT1のウィンドウハンドルhEditを取得.
							GetWindowText(hEdit, tszBuf, 256);	// GetWindowTextでhEditのテキスト内容を取得してtszBufに格納.

							// リストボックスID_LIST1に取得したテキストを追加.
							hList = GetDlgItem(hwndDlg, ID_LIST1);	// GetDlgItemでID_LIST1のウィンドウハンドルhListを取得.
							SendMessage(hList, LB_ADDSTRING, 0, (LPARAM)tszBuf);	// SendMessageのLB_ADDSTRINGでhListにtszBufを追加.
					...
					case ID_BUTTON2:
						...
						// ID_BUTTON2ブロック
						{
							...
							// 変数・配列の宣言
							HWND hList;			// ID_LIST1のウィンドウハンドルhList.
							int idx;			// ID_LIST1の選択された項目のインデックス(位置)idx.
							TCHAR tszTmp[128];	// 一時的なバッファtszTmp.
							
							// 選択された項目のインデックスを取得し, 表示.
							hList = GetDlgItem(hwndDlg, ID_LIST1);	// GetDlgItemでID_LIST1のウィンドウハンドルhListを取得.
							idx = (int)SendMessage(hList, LB_GETCURSEL, 0, 0);	// SendMessageのLB_GETCURSELでhListの選択された項目のインデックスを取得.
							_stprintf(tszTmp, _T("idx = %d"), idx);	// _stprintfでidxを文字列に変換.
							MessageBox(NULL, tszTmp, _T("LB_INSERTSTRING"), MB_OK | MB_ICONASTERISK);	// MessageBoxでtszTmpを表示.
					...
					case ID_BUTTON3:
						...
						// ID_BUTTON3ブロック
						{
							...
							// 選択された項目のインデックスを取得し, そこにエディットボックスID_EDIT1のテキストを挿入.
							hList = GetDlgItem(hwndDlg, ID_LIST1);	// GetDlgItemでID_LIST1のウィンドウハンドルhListを取得.
							idx = (int)SendMessage(hList, LB_GETCURSEL, 0, 0);	// SendMessageのLB_GETCURSELでhListの選択された項目のインデックスを取得.
							hEdit = GetDlgItem(hwndDlg, ID_EDIT1);	// GetDlgItemでID_EDIT1のウィンドウハンドルhEditを取得.
							GetWindowText(hEdit, tszBuf, 256);	// GetWindowTextでhEditのテキスト内容を取得してtszBufに格納.
							SendMessage(hList, LB_INSERTSTRING, idx, (LPARAM)tszBuf);	// SendMessageのLB_INSERTSTRINGでhListのidxの位置にtszBufを挿入.
