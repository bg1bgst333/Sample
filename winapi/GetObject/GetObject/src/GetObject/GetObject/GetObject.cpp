...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetObject");					// ウィンドウクラス名は"GetObject".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetObject"), _T("GetObject"), WS_OVERLAPPEDWINDOW | WS_HSCROLL | WS_VSCROLL, 0, 0, 480, 360, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetObject"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
				...
				if (iRet <= 0){	// 失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("GetObject failed!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"GetObject failed!"とエラーメッセージを表示.
				...
				if (hDC == NULL){	// NULLなら.
					...
					// エラー処理
					MessageBox(hwnd, _T("GetDC failed!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"GetDC failed!"とエラーメッセージを表示.
				...
				if (hMemDC == NULL){	// NULLでなければ.
					...
					// エラー処理
					MessageBox(hwnd, _T("CreateCompatibleDC failed!"), _T("GetObject"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateCompatibleDC failed!"とエラーメッセージを表示.
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押された時.(uMsgがWM_LBUTTONDOWNの時.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				// ウィンドウ名にサイズ情報をセット.
				TCHAR tszSize[32] = {0};	// TCHAR文字配列tszSize(長さ32)を{0}で初期化.
				_stprintf(tszSize, _T("(%d, %d)"), bitmap.bmWidth, bitmap.bmHeight);	// bitmap.bmWidthとbitmap.bmHeightを文字列に変換.
				SetWindowText(hwnd, tszSize);	// SetWindowTextでhwndにウィンドウ名としてtszSizeをセット.
