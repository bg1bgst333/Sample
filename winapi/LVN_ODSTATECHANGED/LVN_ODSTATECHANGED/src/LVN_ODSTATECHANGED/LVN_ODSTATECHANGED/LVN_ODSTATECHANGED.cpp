...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LVN_ODSTATECHANGED");				// ウィンドウクラス名は"LVN_ODSTATECHANGED".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("LVN_ODSTATECHANGED"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("LVN_ODSTATECHANGED"), _T("LVN_ODSTATECHANGED"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"LVN_ODSTATECHANGED"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("LVN_ODSTATECHANGED"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case LVN_ODSTATECHANGED:	// オーナーデータで状態変更時.
							...
							// LVN_ODSTATECHANGEDブロック.
							{
								...
								// デバッグ出力
								OutputDebugString(_T("LVN_ODSTATECHANGED\n"));	// OutputDebugStringで"LVN_ODSTATECHANGED"を出力.
								...
								if (lvi->item.mask & LVIF_TEXT){
									if (lvi->item.iItem == 0){
										if (lvi->item.iSubItem == 0){
											_tcscpy(lvi->item.pszText, _T("item0"));
										}
									...
									if (lvi->item.iItem == 1){
										if (lvi->item.iSubItem == 0){
											_tcscpy(lvi->item.pszText, _T("item1"));
										}
										if (lvi->item.iSubItem == 1){
									...
									if (lvi->item.iItem == 2){
										if (lvi->item.iSubItem == 0){
											_tcscpy(lvi->item.pszText, _T("item2"));
										}
										if (lvi->item.iSubItem == 1){
