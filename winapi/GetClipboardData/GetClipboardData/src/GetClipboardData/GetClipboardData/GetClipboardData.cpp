...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetClipboardData");		//ウィンドウクラス名は"GetClipboardData".
	...
	if (!RegisterClass(&wc)) {	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("GetClipboardData"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetClipboardData"), _T("GetClipboardData"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetClipboardData"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL) {	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("GetClipboardData"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_PASTE:
						...
						// ID_ITEM_PASTEブロック
						{
							...
							HGLOBAL hGlobal;	// グローバルハンドルhGlobal.
							BOOL bOpen;	// 開いたかどうかの結果bOpen.
							PSTR pszText;	// 取得したテキストのポインタpszText.
							int iLen;	// バッファに必要な長さiLen.
							TCHAR *ptszBuf;	// TCHARバッファポインタptszBuf.
							HWND hEdit1;	// ID_EDIT1のハンドルhEdit1.
							...
							if (IsClipboardFormatAvailable(CF_TEXT)){	// IsClipboardFormatAvailableでCF_TEXTがあるか確認.
								...
								bOpen = OpenClipboard(hwnd);	// OpenClipboardでクリップボードを開く.
								if (bOpen){	// 成功.
									hGlobal = GetClipboardData(CF_TEXT);	// GetClipboardDataでハンドルを取得.
									if (hGlobal){	// hGlobalがNULLでない.
										pszText = (PSTR)GlobalLock(hGlobal);	// ロックしてpszTextを取得.
										// マルチバイト文字列をワイド文字列に変換.
										iLen = MultiByteToWideChar(CP_ACP, 0, (char *)pszText, -1, NULL, 0);	// MultiByteToWideCharで必要なバッファの長さを求める.
										ptszBuf = new TCHAR[iLen];	// 長さiLenのTCHARバッファを確保し, ptszBufに格納.
										MultiByteToWideChar(CP_ACP, 0, (char *)pszText, -1, ptszBuf, iLen);	// MultiByteToWideCharでpszTextをptszBufに変換.
										GlobalUnlock(hGlobal);	// アンロック.
										// エディットコントロールにセット.
										hEdit1 = GetDlgItem(hwnd, ID_EDIT1);	// ID_EDIT1のハンドルhEdit1を取得.
										SetWindowText(hEdit1, ptszBuf);	// SetWindowTextでhEdit1にptszBufをセット.							
