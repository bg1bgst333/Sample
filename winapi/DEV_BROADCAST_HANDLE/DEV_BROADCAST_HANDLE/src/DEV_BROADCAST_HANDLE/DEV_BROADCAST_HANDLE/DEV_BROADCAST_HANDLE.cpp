...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("DEV_BROADCAST_HANDLE");			// ウィンドウクラス名は"DEV_BROADCAST_HANDLE".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("DEV_BROADCAST_HANDLE"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("DEV_BROADCAST_HANDLE"), _T("DEV_BROADCAST_HANDLE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"DEV_BROADCAST_HANDLE"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("DEV_BROADCAST_HANDLE"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
									...
									// DBT_DEVTYP_VOLUMEブロック
									{
										...
										if (hDirectory != INVALID_HANDLE_VALUE){	// 成功.
											...
											if (hDevNotify == NULL){	// NULLなら.
												...
												CloseHandle(hDirectory);	// 今回はあえてここで閉じてしまうことにする.
											}
										}
