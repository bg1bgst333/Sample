...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("DOCINFO");						// ウィンドウクラス名は"DOCINFO".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("DOCINFO"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("DOCINFO"), _T("DOCINFO"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"DOCINFO"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("DOCINFO"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_PRINT:
						...
						// ID_ITEM_PRINTブロック
						{
							...
							di.lpszDocName = _T("DOCINFO");	// ドキュメント名は"DOCINFO"とする.
							...
							if (hPrinterDC != NULL){	// hPrinterDCがNULLでなければ.
								...
								if (iRetSD > 0){	// 印刷開始成功.
									...
									if (iRetSP > 0){	// ページ開始成功.
										TextOut(hPrinterDC, 200, 200, _T("ABCDE"), _tcslen(_T("ABCDE")));	// TextOutでプリンタの(200, 200)の位置に"ABCDE"を描画.
