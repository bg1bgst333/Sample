...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("GetMenuState");			// ウィンドウクラス名は"GetMenuState".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("GetMenuState"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("GetMenuState"), _T("GetMenuState"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"GetMenuState"ウィンドウクラスのウィンドウを作成.(9番目にNULLを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("GetMenuState"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
		...
		case WM_DESTROY:	// ウィンドウが破棄された時.(uMsgがWM_DESTROYの時.)
			...
			// WM_DESTROYブロック
			{
				...
				if (hMenu != NULL){	// NULLでない.
					...
					if (bRet){
						MessageBox(hwnd, _T("DestroyMenu success!"), _T("GetMenuState"), MB_OK);	// "DestroyMenu success!"と表示.
					}
					else{
						MessageBox(hwnd, _T("DestroyMenu failed!"), _T("GetMenuState"), MB_OK);	// "DestroyMenu failed!"と表示.
					}
				}
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							if (uiRet & MF_DISABLED){	// 無効.
								...
								EnableMenuItem(GetMenu(hwnd), ID_ITEM_1_1, MF_DISABLED);
							}
