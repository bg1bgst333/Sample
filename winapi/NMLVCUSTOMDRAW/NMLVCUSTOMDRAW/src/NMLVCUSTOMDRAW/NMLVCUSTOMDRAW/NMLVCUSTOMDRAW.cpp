...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("NMLVCUSTOMDRAW");					// ウィンドウクラス名は"NMLVCUSTOMDRAW".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("NMLVCUSTOMDRAW"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("NMLVCUSTOMDRAW"), _T("NMLVCUSTOMDRAW"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"NMLVCUSTOMDRAW"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("NMLVCUSTOMDRAW"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				// リストビューの拡張スタイルを取得.
				DWORD dwListViewExStyle = ListView_GetExtendedListViewStyle(hList);	// ListView_GetExtendedListViewStyleでリストビューの拡張スタイルを取得.
				TCHAR tszStyle[256] = {0};
				_stprintf(tszStyle, _T("dwListViewExStyle = 0x%08x"), dwListViewExStyle);
				OutputDebugString(tszStyle);
				// ホットトラックの拡張スタイルも設定.
				ListView_SetExtendedListViewStyle(hList, dwListViewExStyle | LVS_EX_CHECKBOXES | LVS_EX_TRACKSELECT | LVS_EX_ONECLICKACTIVATE);	// ListView_SetExtendedListViewStyleでLVS_EX_TRACKSELECTとLVS_EX_ONECLICKACTIVATEも追加.
		...
		case WM_NOTIFY:
			...
			// WM_NOTIFYブロック
			{
				...
				if ((int)wParam == IDC_LIST1){	// wParamがIDC_LIST1の時.
					...
					NMLVCUSTOMDRAW *cd = (NMLVCUSTOMDRAW *)lParam;	// lParamをNMLVCUSTOMDRAWのポインタに変換.
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case LVN_HOTTRACK:	// ホットトラック.
							...
							// LVN_HOTTRACKブロック.
							{
								...
								ScreenToClient(lv->hdr.hwndFrom, &pt);
								LVHITTESTINFO lvhti = {0};
								lvhti.pt = pt;
								int iIndex = ListView_SubItemHitTest(lv->hdr.hwndFrom, &lvhti);	// ListView_SubItemHitTestでマウスがいる位置の行と列を取得.
								if (iIndex != -1){
									TCHAR tszBuf[512] = {0};
									_stprintf(tszBuf, _T("ListView_SubItemHitTest: iItem = %d, iSubItem = %d\n"), lvhti.iItem, lvhti.iSubItem);
						...
						case NM_KILLFOCUS:	// フォーカスを失った時.
							...
							break;
						case NM_CUSTOMDRAW:	// カスタムドロー時.

							// NM_CUSTOMDRAWブロック.
							{

								// cd->nmcd.dwDrawStageごとに振り分け.
								switch (cd->nmcd.dwDrawStage){
									...
									case CDDS_ITEMPREPAINT:
										cd->clrTextBk = RGB(0, 255, 0);	// テキスト背景色を緑に.
									...
									case CDDS_ITEMPREPAINT | CDDS_SUBITEM:
										if (cd->iSubItem % 2 == 1){
											cd->clrTextBk = RGB(255, 0, 0);	// テキスト背景色を赤に.
										}
										else{
											cd->clrTextBk = RGB(0, 255, 0);	// テキスト背景色を緑に.
										}
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON6:
						...
						// ID_BUTTON6ブロック
						{
							...
							// 0番目のアイテムのチェックボックスにチェックを点ける.
							ListView_SetCheckState(hList, 0, TRUE);	// ListView_SetCheckStateで0番目にチェック.
					...
					case ID_BUTTON7:
						...
						// ID_BUTTON7ブロック
						{
							...
							// アイテムの数を取得する.
							int c = ListView_GetItemCount(hList);	// ListView_GetItemCountでアイテムの数を取得.
							TCHAR tszCount[256] = {0};
							_stprintf(tszCount, _T("c = %d\n"), c);
							OutputDebugString(tszCount);
							
							// どれがチェックされているか調べる.
							for (int i = 0; i < c; i++){
								BOOL bCheck = ListView_GetCheckState(hList, i);	// ListView_GetCheckStateでチェックされているか調べる.
								TCHAR tszCheck[256] = {0};
								if (bCheck){
									_stprintf(tszCheck, _T("i = %d, Checked!\n"), i);
								}
								else{
									_stprintf(tszCheck, _T("i = %d, Not checked!\n"), i);
								}
								OutputDebugString(tszCheck);
							}
		...
		// マウスが移動した時.
		case WM_MOUSEMOVE:	// マウスが移動した時.(uMsgがWM_MOUSEMOVEの時.)
