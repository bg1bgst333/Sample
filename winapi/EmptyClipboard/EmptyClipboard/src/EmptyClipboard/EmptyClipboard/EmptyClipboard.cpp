...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("EmptyClipboard");				// ウィンドウクラス名は"EmptyClipboard".
	...
	if (!RegisterClass(&wc)) {	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("EmptyClipboard"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("EmptyClipboard"), _T("EmptyClipboard"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"EmptyClipboard"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL) {	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("EmptyClipboard"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					case ID_ITEM_COPY:
						...
						// ID_ITEM_COPYブロック
						{
							...
							if (bOpen){	// 成功.
								...
								if (bEmpty){	// 成功.
									MessageBox(hwnd, _T("EmptyClipboard Success!"), _T("EmptyClipboard"), MB_OK | MB_ICONASTERISK);	// "EmptyClipboard Success!"と表示.
								}
								else{	// 失敗.
									MessageBox(hwnd, _T("EmptyClipboard Failure!"), _T("EmptyClipboard"), MB_OK | MB_ICONHAND);	// "EmptyClipboard Failure!"と表示.
								}
