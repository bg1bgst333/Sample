...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_DragShowNolock");				// ウィンドウクラス名は"ImageList_DragShowNolock".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ってきたらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ImageList_DragShowNolock"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_DragShowNolock"), _T("ImageList_DragShowNolock"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ImageList_DragShowNolock"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ImageList_DragShowNolock"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){	// ウィンドウメッセージに対して独自の処理ができるように定義したウィンドウプロシージャ.
	...
	static RECT rcForbidden;				// ImageList_DragShowNolockで表示/非表示を切り替える禁止エリアを表すRECT型スタティック変数rcForbidden. rcLimit内右側に配置.
	...
	switch (uMsg){	// switch-case文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始されたとき.(uMsgがWM_CREATEの場合.)
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap == NULL){	// hBitmapがNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_DragShowNolock"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
					return -1;	// ウィンドウ作成失敗.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_DragShowNolock"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
		...
		case WM_PAINT:		// 画面の描画が求められたとき.(uMsgがWM_PAINTの場合.)
			...
			// WM_PAINTブロック
			{
				...
				// 禁止エリアを塗り潰す. ドラッグ画像を隠している間(bImageHidden)は濃い色, それ以外は薄い色.
		...
		case WM_MOUSEMOVE:	// マウスが移動したとき.(uMsgがWM_MOUSEMOVEの場合.)
			...
			// WM_MOUSEMOVEブロック
			{
				...
				if (bDragging){
					...
					// ImageList_DragShowNolockはウィンドウのロックを外さないので,
					// 非表示中でもImageList_DragMoveを呼び続けて位置を追従させられる.
					// (ImageList_DragLeaveを使う場合は, 再表示のたびにImageList_DragEnterで
					//  座標を指定し直す必要があったが, DragShowNolockではその必要がない.)
					ImageList_DragMove(ptWin.x, ptWin.y);	// ImageList_DragMoveでドラッグ画像を(ptWin.x, ptWin.y)(ウィンドウ内座標)の位置に移動.

					// ドラッグ画像の矩形が禁止エリア(rcForbidden)と少しでも重なっているかどうかを判定.
					if (IsImageOverlapForbidden(pt, &rcForbidden)){	// 禁止エリアと重なっている場合.

						if (!bImageHidden){	// まだ隠していない場合(禁止エリアに触れた瞬間).

							ImageList_DragShowNolock(FALSE);	// ImageList_DragShowNolockにFALSEを渡してドラッグ画像を隠す.(ウィンドウのロックは外れない.)
						...
						if (bImageHidden){	// 隠していた場合(禁止エリアから出た瞬間).

							ImageList_DragShowNolock(TRUE);	// ImageList_DragShowNolockにTRUEを渡してドラッグ画像を再表示.(座標の指定は不要.)
				...
				if (PtInRect(&rcLimit, pt)){
					...
					if (ImageList_BeginDrag(hImageList, 0, 0, 0)){	// ImageList_BeginDragでhImageListの0番目の画像をドラッグ開始. ホットスポットは画像左上(0, 0).
						...
						if (ImageList_DragEnter(hwnd, ptWin.x, ptWin.y)){	// ImageList_DragEnterでhwndをロックしてドラッグ画像をクリック位置(ptWin)(ウィンドウ内座標)に表示.
							...
							// クリックした時点で既に禁止エリアと重なっている場合は, DragEnter直後に隠す.
							if (IsImageOverlapForbidden(pt, &rcForbidden)){

								ImageList_DragShowNolock(FALSE);	// ImageList_DragShowNolockにFALSEを渡してドラッグ画像を隠す.
								bImageHidden = TRUE;		// bImageHiddenをTRUEに設定.
								...
								bImageHidden = FALSE;	// bImageHiddenをFALSEに設定.(表示したまま.)
							}
		...
		case WM_LBUTTONUP:	// マウスの左ボタンが離されたとき.(uMsgがWM_LBUTTONUPの場合.)
			...
			// WM_LBUTTONUPブロック
			{
				...
				// ドラッグ中なら終了.
				if (bDragging){
					ImageList_DragLeave(hwnd);	// ImageList_DragLeaveでhwndのロックを解除.(隠れていても表示中でも, 最後は必ずDragLeaveでロック解除する.)
					ImageList_EndDrag();		// ImageList_EndDragでドラッグを終了.
