...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WM_CTLCOLOREDIT");					// ウィンドウクラス名は"WM_CTLCOLOREDIT".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("WM_CTLCOLOREDIT"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("WM_CTLCOLOREDIT"), _T("WM_CTLCOLOREDIT"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WM_CTLCOLOREDIT"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("WM_CTLCOLOREDIT"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_CTLCOLOREDIT:
			...
			// WM_CTLCOLOREDITブロック
			{
				...
				// ブラシハンドル作成.
				if (hbrEditBackground == NULL){
					hbrEditBackground = CreateSolidBrush(RGB(255, 0, 0));	// 赤
				}
				...
				if (hwndCtrl == hEdit1){	// IDC_BUTTON1
					SetBkColor(hDC, RGB(255, 0, 0));	// 赤
					SetTextColor(hDC, RGB(255, 255, 255));	// 白
					return (INT_PTR)hbrEditBackground;	// 赤のブラシを返す.
				}
				else{
