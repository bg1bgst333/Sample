...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CCHookProc");						// ウィンドウクラス名"CCHookProc".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("CCHookProc"), _T("CCHookProc"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
...
UINT CALLBACK CCHookProc(HWND hDlg, UINT uiMsg, WPARAM wp, LPARAM lp){
	switch (uiMsg){
		case WM_INITDIALOG:
			SetWindowText(hDlg, _T("CCHookProc!!!"));
			return TRUE;
	}
	return FALSE;
}