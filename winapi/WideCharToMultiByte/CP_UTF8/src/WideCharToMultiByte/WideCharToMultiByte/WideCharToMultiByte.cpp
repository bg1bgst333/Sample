				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_SAVE:
						...
						// ID_ITEM_SAVEブロック
						{
							...
							// ワイド文字をマルチバイト文字(UTF-8)に変換する.
							iBufLen = WideCharToMultiByte(CP_UTF8, 0, ptszBuf, -1, NULL, 0, NULL, NULL);	// まずは長さを取得.
							pszBuf = new char[iBufLen];	// iBufLenのchar型バッファを確保.
							WideCharToMultiByte(CP_UTF8, 0, ptszBuf, -1, pszBuf, iBufLen, NULL, NULL);	// 変換.
