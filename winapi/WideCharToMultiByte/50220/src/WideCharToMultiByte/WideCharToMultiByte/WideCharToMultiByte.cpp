...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("WideCharToMultiByte");			// ウィンドウクラス名は"WideCharToMultiByte".
	...
	if (!RegisterClass(&wc)) {	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("WideCharToMultiByte"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("WideCharToMultiByte"), _T("WideCharToMultiByte"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"WideCharToMultiByte"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL) {	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("WideCharToMultiByte"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_OPEN:
						...
						// ID_ITEM_OPENブロック
						{
							...
							if (hFile != INVALID_HANDLE_VALUE){	// hFileがINVALID_HANDLE_VALUEでなければ.
								...
								// マルチバイト文字列(JIS(ISO-2022-JP(50220)))をワイド文字列に変換.
								iLen = MultiByteToWideChar(50220, 0, (char *)btBuf, -1, NULL, 0);	// MultiByteToWideCharで必要なバッファの長さを求める.
								ptszBuf = new TCHAR[iLen];	// 長さiLenのTCHARバッファを確保し, ptszBufに格納.
								MultiByteToWideChar(50220, 0, (char *)btBuf, -1, ptszBuf, iLen);	// MultiByteToWideCharでbtBufをptszBufに変換.
					...
					case ID_ITEM_SAVE:
						...
						// ID_ITEM_SAVEブロック
						{
							...
							// ワイド文字をマルチバイト文字(JIS(ISO-2022-JP(50220)))に変換する.
							iBufLen = WideCharToMultiByte(50220, 0, ptszBuf, -1, NULL, 0, NULL, NULL);	// まずは長さを取得.
							pszBuf = new char[iBufLen];	// iBufLenのchar型バッファを確保.
							WideCharToMultiByte(50220, 0, ptszBuf, -1, pszBuf, iBufLen, NULL, NULL);	// 変換.
					...
					case ID_ITEM_COPY:
						...
						// ID_ITEM_COPYブロック
						{
							...
							if (bOpen){	// 成功.
								...
								if (bEmpty){	// 成功.
									MessageBox(hwnd, _T("EmptyClipboard Success!"), _T("WideCharToMultiByte"), MB_OK | MB_ICONASTERISK);	// "EmptyClipboard Success!"と表示.
								}
								else{	// 失敗.
									MessageBox(hwnd, _T("EmptyClipboard Failure!"), _T("WideCharToMultiByte"), MB_OK | MB_ICONHAND);	// "EmptyClipboard Failure!"と表示.
								}
