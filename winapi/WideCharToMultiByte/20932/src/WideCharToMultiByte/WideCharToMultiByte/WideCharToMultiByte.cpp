				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_OPEN:
						...
						// ID_ITEM_OPENブロック
						{
							...
							if (hFile != INVALID_HANDLE_VALUE){	// hFileがINVALID_HANDLE_VALUEでなければ.
								...
								// マルチバイト文字列(EUC-JP(20932))をワイド文字列に変換.
								iLen = MultiByteToWideChar(20932, 0, (char *)btBuf, -1, NULL, 0);	// MultiByteToWideCharで必要なバッファの長さを求める.
								ptszBuf = new TCHAR[iLen];	// 長さiLenのTCHARバッファを確保し, ptszBufに格納.
								MultiByteToWideChar(20932, 0, (char *)btBuf, -1, ptszBuf, iLen);	// MultiByteToWideCharでbtBufをptszBufに変換.
					...
					case ID_ITEM_SAVE:
						...
						// ID_ITEM_SAVEブロック
						{
							...
							// ワイド文字をマルチバイト文字(EUC-JP(20932))に変換する.
							iBufLen = WideCharToMultiByte(20932, 0, ptszBuf, -1, NULL, 0, NULL, NULL);	// まずは長さを取得.
							pszBuf = new char[iBufLen];	// iBufLenのchar型バッファを確保.
							WideCharToMultiByte(20932, 0, ptszBuf, -1, pszBuf, iBufLen, NULL, NULL);	// 変換.
