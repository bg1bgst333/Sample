...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("KillTimer");						// ウィンドウクラス名は"KillTimer".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("KillTimer"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("KillTimer"), _T("KillTimer"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"KillTimer"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("KillTimer"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
		...
		case WM_COMMAND:	// メニュー項目が選択されたり, ボタンなどのコントロールが押されたりして, コマンドが発生した時.(uMsgがWM_COMMANDの時.)
			...
			// WM_COMMANDブロック
			{
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					// Timer1のStopが選択された時.
					case ID_ITEM_TIMER1_STOP:

						// ID_ITEM_TIMER1_STOPブロック
						{

							// タイマーの破棄
							KillTimer(hwnd, ID_TIMER1);	// KillTimerでタイマーを破棄.
					...
					// Timer2のStopが選択された時.
					case ID_ITEM_TIMER2_STOP:

						// ID_ITEM_TIMER2_STOPブロック
						{

							// 10秒ごとのタイマーの破棄
							KillTimer(hwnd, ID_TIMER2);	// KillTimerで10秒ごとのタイマーを破棄.
