			...
			if (rc != 0){	// rcが0以外なら成功.(AddJobの戻り値は本来BOOLだが、int型rcで取っている.この謎は別途記事を参照.)
				...
				if (!(rc != 122 && dwNeeded < 1)){	// rcに122(ERROR_INSUFFICIENT_BUFFER)が返ってくるか, dwNeededが正の値なら, 全体は真となり, バッファサイズ取得としては成功.
					...
					if (rc != 0){	// rcが0以外なら成功.
						...
						// ジョブを一時停止.
						SetJob(hPrinter, aji1->JobId, 1, (LPBYTE)ji1, JOB_CONTROL_PAUSE);	// SetJobでji1とJOB_CONTROL_PAUSEをセット.
						MessageBox(NULL, _T("Debug Stop!"), _T("SetJob"), MB_OK);	// MessageBoxでデバッグ用に一旦処理をとめる.
					}
					delete[] ji1;	// ji1を解放.
				}
