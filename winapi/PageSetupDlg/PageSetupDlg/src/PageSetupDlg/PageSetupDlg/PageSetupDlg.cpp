...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("PageSetupDlg");					// ウィンドウクラス名は"PageSetupDlg".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("PageSetupDlg"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
 
	}

	// ウィンドウの作成.
	hWnd = CreateWindow(_T("PageSetupDlg"), _T("PageSetupDlg"), WS_OVERLAPPEDWINDOW, 100, 100, 640, 480, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"PageSetupDlg"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.
 
		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("PageSetupDlg"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					case ID_ITEM_PRINT:
						...
						// ID_ITEM_PRINTブロック
						{
							...
							if (bRet){	// 印刷を押したらTRUE.
								...
								if (bRet){	// bRetがTRUE.
									MessageBox(NULL, _T("OK!"), _T("PageSetupDlg"), MB_OK);	// MessageBoxで"OK!"とメッセージを表示.
								}
								else{	// bRetがFALSE.
									MessageBox(NULL, _T("Cancel!"), _T("PageSetupDlg"), MB_OK);	// MessageBoxで"Cancel!"とメッセージを表示.
								}
