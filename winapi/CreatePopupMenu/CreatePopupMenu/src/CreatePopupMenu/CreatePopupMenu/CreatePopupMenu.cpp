...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	HMENU hMenu = NULL;			// このウィンドウのメインメニューハンドルhMenuをNULLに初期化.
	HMENU hPopupMenu1 = NULL;	// hMenuの子メニューとなるポップアップメニューのハンドルhPopupMenu1をNULLに初期化.
	HMENU hPopupMenu2 = NULL;	// hMenuの子メニューとなるポップアップメニューのハンドルhPopupMenu2をNULLに初期化.
	HMENU hPopupMenu3 = NULL;	// hMenuの子メニューとなるポップアップメニューのハンドルhPopupMenu3をNULLに初期化.

	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CreatePopupMenu");				// ウィンドウクラス名は"CreatePopupMenu".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("CreatePopupMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)
	...
	AppendMenu(hPopupMenu2, MF_ENABLED | MF_STRING, ID_ITEM_2_2, _T("Item2-2(&G)"));	// AppendMenuでhPopupMenu2にメニュー項目Item2-2を追加.
	AppendMenu(hPopupMenu2, MF_ENABLED | MF_STRING, ID_ITEM_2_3, _T("Item2-3(&H)"));	// AppendMenuでhPopupMenu2にメニュー項目Item2-3を追加.
	...
	// メニューhMenuにポップアップメニューhPopupMenu1, hPopupMenu2, hPopupMenu3を追加.
	AppendMenu(hMenu, MF_ENABLED | MF_POPUP, (UINT_PTR)hPopupMenu1, _T("Item1(&A)"));	// AppendMenuでhMenuにhPopupMenu1を追加.
	AppendMenu(hMenu, MF_ENABLED | MF_POPUP, (UINT_PTR)hPopupMenu2, _T("Item2(&E)"));	// AppendMenuでhMenuにhPopupMenu2を追加.
	AppendMenu(hMenu, MF_ENABLED | MF_POPUP, (UINT_PTR)hPopupMenu3, _T("Item3(&I)"));	// AppendMenuでhMenuにhPopupMenu3を追加.
		
	// ウィンドウの作成
	hWnd = CreateWindow(_T("CreatePopupMenu"), _T("CreatePopupMenu"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"CreatePopupMenu"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("CreatePopupMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
