...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ImageList_Add");						// ウィンドウクラス名"ImageList_Add".
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("ImageList_Add"), _T("ImageList_Add"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウ作成.
	if (hWnd == NULL){
	...
	switch (uMsg){	// uMsgごとに処理振り分け.
		...
		case WM_CREATE:
			...
			// WM_CREATEブロック
			{
				...
				if (hBitmap1 == NULL){	// hBitmap1がNULLならロード失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("LoadImage failed!"), _T("ImageList_Add"), MB_OK | MB_ICONHAND);	// MessageBoxで"LoadImage failed!"とエラーメッセージを表示.
				...
				if (hImageList == NULL){	// hImageListがNULLなら失敗.
					...
					// エラー処理
					MessageBox(hwnd, _T("ImageList_Create failed!"), _T("ImageList_Add"), MB_OK | MB_ICONHAND);	// MessageBoxで"ImageList_Create failed!"とエラーメッセージを表示.
		...
		case WM_PAINT:
			...
			// WM_PAINTブロック
			{
				...
				// 描画.
				ImageList_Draw(hImageList, 0, hPaintDC, 50, 50, ILD_NORMAL);
				ImageList_Draw(hImageList, 1, hPaintDC, 50 + 32, 50, ILD_NORMAL);
