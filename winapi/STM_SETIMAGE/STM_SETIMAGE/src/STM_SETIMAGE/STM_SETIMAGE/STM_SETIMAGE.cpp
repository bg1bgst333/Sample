...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("STM_SETIMAGE");					// ウィンドウクラス名は"STM_SETIMAGE".
	...
	if (!RegisterClass(&wc)) {	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("STM_SETIMAGE"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("STM_SETIMAGE"), _T("STM_SETIMAGE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"STM_SETIMAGE"ウィンドウクラスのウィンドウを作成.
	...
	switch (uMsg) {	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				// スタティックコントロールの作成.
				hStatic = CreateWindow(WC_STATIC, _T(""), WS_CHILD | WS_VISIBLE | SS_BITMAP, 50, 50, 200, 200, hwnd, (HMENU)WM_APP + 1, lpCS->hInstance, NULL);	// CreateWindowのWC_STATICでスタティックコントロールを作成.
