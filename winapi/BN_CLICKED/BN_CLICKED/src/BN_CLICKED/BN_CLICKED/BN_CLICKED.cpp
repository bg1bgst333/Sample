...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("BN_CLICKED");							// ウィンドウクラス名は"BN_CLICKED".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("BN_CLICKED"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("BN_CLICKED"), _T("BN_CLICKED"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"BN_CLICKED"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("BN_CLICKED"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CLOSE:	// ウィンドウが閉じられた時.(uMsgがWM_CLOSEの時.)
			...
			// WM_CLOSEブロック
			{
				...
				int iRet = MessageBox(hwnd, _T("Close?"), _T("BN_CLICKED"), MB_ICONQUESTION | MB_OKCANCEL);	// MessageBoxで"Close?"を表示し, MB_OKCANCELの結果をiRetに格納.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目, またはコントロールのIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_1:
						...
						// ID_ITEM_1_1ブロック
						{
							...
							// メニューアイテムの場合, 通知コードはないというか常に0.
							if (HIWORD(wParam) == 0){	// HIWORD(wParam)は0.

								// HIWORD(wParam)をデバッグ出力.
								TCHAR tszNotificationCodeStr[128];	// 通知コード文字列.
								_stprintf(tszNotificationCodeStr, _T("HIWORD(wParam) = 0x%08x\n"), HIWORD(wParam));	// HIWORD(wParam)を文字列に変換.
								OutputDebugString(tszNotificationCodeStr);	// OutputDebugStringでtszNotificationCodeStrをデバッグ出力.
					...
					case ID_BUTTON1:
						...
						// ID_BUTTON1ブロック
						{
							...
							// ボタンの場合, クリックされた時の通知コードはBN_CLICKED( == 0).
							if (HIWORD(wParam) == BN_CLICKED){	// HIWORD(wParam)はBN_CLICKED( == 0).

								// HIWORD(wParam)をデバッグ出力.
								TCHAR tszNotificationCodeStr[128];	// 通知コード文字列.
								_stprintf(tszNotificationCodeStr, _T("BN_CLICKED: HIWORD(wParam) = 0x%08x\n"), HIWORD(wParam));	// HIWORD(wParam)を文字列に変換.
								OutputDebugString(tszNotificationCodeStr);	// OutputDebugStringでtszNotificationCodeStrをデバッグ出力.
