...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	MessageBox(NULL, tszHInstance, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszHInstanceを表示.
	...
	switch (uMsg) {	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				_stprintf(tszHInstance, _T("CREATESTRUCT.hInstance = 0x%08x"), (unsigned long)lpCreateStruct->hInstance);	// lpCreateStruct->hInstanceを文字列に変換.
				MessageBox(NULL, tszHInstance, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszHInstanceを表示.

				// 子ウィンドウの作成(子ウィンドウなので明示的な破棄は不要.)
				CreateWindow(_T("Child_Window"), _T("CREATESTRUCT"), WS_CHILD | WS_VISIBLE | WS_BORDER, 100, 100, 200, 200, hwnd, (HMENU)WM_APP + 1, lpCreateStruct->hInstance, NULL);	// CreateWindowでhwndを親とする子ウィンドウの作成.(lpCreateStruct->hInstanceを指定.)
