...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	MessageBox(NULL, tszWndProc, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszWndProcを表示.
	...
	MessageBox(NULL, tszWndProc2, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszWndProc2を表示.
	...
	// ウィンドウの作成
	hWnd = CreateWindow(_T("CREATESTRUCT"), _T("CREATESTRUCT"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"CREATESTRUCT"ウィンドウクラスのウィンドウを作成.
	...
	switch (uMsg) {	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				TCHAR tszHWnd[256] = {0};		// ウィンドウハンドル文字列tszHWnd(長さ256)を{0}で初期化.
				...
				// ウィンドウハンドル文字列の表示.
				_stprintf(tszHWnd, _T("WindowProc#hwnd = 0x%08x"), (unsigned long)hwnd);	// hwndを文字列に変換.
				MessageBox(NULL, tszHWnd, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszHWndを表示.

				// アプリケーションインスタンスハンドルの取得と子ウィンドウの作成.
				lpCreateStruct = (LPCREATESTRUCT)lParam;	// lParamをLPCREATESTRUCTにキャストし, lpCreateStructに格納.
				hWnd2 = CreateWindow(_T("Child_Window"), _T("CREATESTRUCT"), WS_CHILD | WS_VISIBLE | WS_BORDER, 100, 100, 200, 200, hwnd, (HMENU)WM_APP + 1, lpCreateStruct->hInstance, NULL);	// CreateWindowでhwndを親とする子ウィンドウの作成し, ウィンドウハンドルをhWnd2に格納.(lpCreateStruct->hInstanceを指定.最後の引数にNULLを渡す.)
	...
	switch (uMsg) {	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				TCHAR tszWndProc[256] = {0};	// ウィンドウプロシージャ文字列tszWndProc(長さ256)を{0}で初期化.
				TCHAR tszWndProc2[256] = {0};	// ウィンドウプロシージャ文字列tszWndProc2(長さ256)を{0}で初期化.
				TCHAR tszHWndParent[256] = {0};		// 親ウィンドウハンドル文字列tszHWndParent(長さ256)を{0}で初期化.

				// lParamをlpCreateStructにキャスト.
				lpCreateStruct = (LPCREATESTRUCT)lParam;	// lParamをLPCREATESTRUCTにキャストし, lpCreateStructに格納.

				// 親ウィンドウプロシージャアドレス文字列の表示.
				wndProc = (WNDPROC)GetWindowLong(lpCreateStruct->hwndParent, GWL_WNDPROC);	// GetWindowLongにGWL_WNDPROCを指定してwndProcを取得.(親ウィンドウハンドルはlpCreateStruct->hwndParentに格納されている.)
				_stprintf(tszWndProc, _T("WindowProc2#WindowProc = 0x%08x"), (unsigned long)wndProc);	// wndProcを文字列に変換.
				MessageBox(NULL, tszWndProc, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszWndProcを表示.

				// 親ウィンドウハンドル文字列の表示.
				_stprintf(tszHWndParent, _T("WindowProc2#lpCreateStruct->hwndParent = 0x%08x"), (unsigned long)lpCreateStruct->hwndParent);	// lpCreateStruct->hwndParentを文字列に変換.
				MessageBox(NULL, tszHWndParent, _T("CREATESTRUCT"), MB_OK);	// MessageBoxでtszHWndParentを表示.
