...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("CB_INSERTSTRING");					// ウィンドウクラス名は"CB_INSERTSTRING".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("CB_INSERTSTRING"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("CB_INSERTSTRING"), _T("CB_INSERTSTRING"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"CB_INSERTSTRING"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("CB_INSERTSTRING"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
					...
					case IDC_BUTTON1:
						...
						// IDC_BUTTON1ブロック
						{
							...
							if (iLen > 0){	// 0より大きい.
								...
								// 先頭にテキストを挿入.
								SendMessage(hCombo, CB_INSERTSTRING, (WPARAM)0, (LPARAM)ptszText);	// CB_INSERTSTRINGで先頭にテキストを挿入.
