...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("DestroyMenu");					// ウィンドウクラス名は"DestroyMenu".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("DestroyMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("DestroyMenu"), _T("DestroyMenu"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, hMenu, hInstance, NULL);	// CreateWindowで, 上で登録した"DestroyMenu"ウィンドウクラスのウィンドウを作成.(9番目にhMenuを指定する.)
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("DestroyMenu"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_1_1:
						...
						// ID_ITEM_1_1ブロック
						{
							...
							// Item1を削除.
							DeleteMenu(hMenu, 0, MF_BYPOSITION);	// DeleteMenuでhMenuの0番目のItem1を削除.(hPopupMenu1は破棄される.)
							BOOL bRet = DestroyMenu(hPopupMenu1);	// DestroyMenuでhPopupMenu1を破棄.
							if (!bRet){	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopupMenu1) failed!"), _T("DestroyMenu"), MB_OK);	// "DestroyMenu(hPopupMenu1) failed!"と表示.
							}
					...
					case ID_ITEM_1_2:
						...
						// ID_ITEM_1_2ブロック
						{
							...
							// Item2を削除.
							DeleteMenu(hMenu, 1, MF_BYPOSITION);	// DeleteMenuでhMenuの1番目のItem2を削除.(hPopupMenu2は破棄される.)
							BOOL bRet = DestroyMenu(hPopupMenu2);	// DestroyMenuでhPopupMenu2を破棄.
							if (!bRet){	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopupMenu2) failed!"), _T("DestroyMenu"), MB_OK);	// "DestroyMenu(hPopupMenu2) failed!"と表示.
							}
					...
					case ID_ITEM_1_3:
						...
						// ID_ITEM_1_3ブロック
						{
							...
							// Item3を削除.
							DeleteMenu(hMenu, 2, MF_BYPOSITION);	// DeleteMenuでhMenuの2番目のItem3を削除.(hPopupMenu3は破棄される.)
							BOOL bRet = DestroyMenu(hPopupMenu3);	// DestroyMenuでhPopupMenu3を破棄.
							if (!bRet){	// bRetがFALSE.
								MessageBox(hwnd, _T("DestroyMenu(hPopupMenu3) failed!"), _T("DestroyMenu"), MB_OK);	// "DestroyMenu(hPopupMenu3) failed!"と表示.
							}
