	...
	if (hDevInfo != INVALID_HANDLE_VALUE){	// INVALID_HANDLE_VALUEでなければ.
		...
		while (bLoop){	// bLoopがTRUEの間は続ける.
			...
			if (!bRet){	// FALSEなら.
				...
				// プロパティの取得.(今回はクラスプロパティのサイズだけ.)
				DWORD dwRegType;	// プロパティの型.
				DWORD dwSize;	// プロパティのサイズ.
				SetupDiGetDeviceRegistryProperty(hDevInfo, &spdd, SPDRP_CLASS, &dwRegType, NULL, 0, &dwSize);	// SetupDiGetDeviceRegistryPropertyでクラスのサイズ取得.
