...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ReleaseCapture");					// ウィンドウクラス名は"ReleaseCapture".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ReleaseCapture"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ReleaseCapture"), _T("ReleaseCapture"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ReleaseCapture"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ReleaseCapture"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_PAINT:		// 画面の描画を要求された時.(uMsgがWM_PAINTの時.)
			...
			// WM_PAINTブロック
			{
				...
				// 矩形の描画
				RECT rc;
				rc.left = min(ptStart.x, ptEnd.x);
				rc.right = max(ptStart.x, ptEnd.x);
				rc.top = min(ptStart.y, ptEnd.y);
				rc.bottom = max(ptStart.y, ptEnd.y);
		...
		case WM_MOUSEMOVE:
			...
			// WM_MOUSEMOVEブロック
			{
				...
				if (!bTracking){
					...
					tme.hwndTrack = hwnd;
					tme.dwHoverTime = 5000;	// 5秒後に通知が来る.
					TrackMouseEvent(&tme);	// TrackMouseEventでマウスイベントを追跡.
					OutputDebugString(_T("TrackMouseEvent after\n"));
					bTracking = TRUE;
				}
		...
		case WM_LBUTTONDOWN:	// マウスの左ボタンが押された時.(uMsgがWM_LBUTTONDOWNの時.)
			...
			// WM_LBUTTONDOWNブロック
			{
				...
				SetCapture(hwnd);
				TCHAR tszTemp[256] = {0};
				_stprintf(tszTemp, _T("SetCapture: x = %hd, y = %hd\n"), ptStart.x, ptStart.y);
