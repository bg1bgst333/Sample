...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("LVN_HOTTRACK");						// ウィンドウクラス名は"LVN_HOTTRACK".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("LVN_HOTTRACK"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("LVN_HOTTRACK"), _T("LVN_HOTTRACK"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"LVN_HOTTRACK"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("LVN_HOTTRACK"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// uMsgの内容で判断.
		...
		case WM_INITDIALOG:	// ダイアログの初期化時.(uMsgがWM_INITDIALOGの時.)
			...
			// WM_INITDIALOGブロック
			{
				...
				// ホットトラックの拡張スタイルも設定.
				ListView_SetExtendedListViewStyle(hList, dwListViewExStyle | LVS_EX_CHECKBOXES | LVS_EX_TRACKSELECT | LVS_EX_ONECLICKACTIVATE | LVS_EX_FULLROWSELECT);	// ListView_SetExtendedListViewStyleでLVS_EX_TRACKSELECTとLVS_EX_ONECLICKACTIVATEとLVS_EX_FULLROWSELECTも追加.
					...
					switch (hdr->code){	// 通知コードごとに分ける.
						...
						case LVN_HOTTRACK:	// ホットトラック.
							...
							// LVN_HOTTRACKブロック.
							{
								...
								// マウスオーバーしているアイテム, サブアイテム情報を出力.
								TCHAR tszItemSubItemIdx[512] = {0};
								_stprintf(tszItemSubItemIdx, _T("LVN_HOTTRACK: lv->iItem = %d, lv->iSubItem = %d\n"), lv->iItem, lv->iSubItem);
								OutputDebugString(tszItemSubItemIdx);
