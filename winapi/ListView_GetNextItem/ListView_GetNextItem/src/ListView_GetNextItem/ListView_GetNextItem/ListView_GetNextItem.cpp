...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ListView_GetNextItem ");				// ウィンドウクラス名は"ListView_GetNextItem ".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("ListView_GetNextItem "), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ListView_GetNextItem "), _T("ListView_GetNextItem "), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ListView_GetNextItem "ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("ListView_GetNextItem "), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)でコントロールのリソースIDが取得できるので, その値で判定する.
					...
					case ID_BUTTON2:
						...
						// ID_BUTTON2ブロック
						{
							...
							TCHAR tszIdx[16];	// インデックスを文字列に変換したものtszIdx.(長さ16)

							// リストビューのハンドルを取得.
							hList = GetDlgItem(hwndDlg, IDC_LIST1);	// GetDlgItemでIDC_LIST1のハンドル取得.

							// 選択されている項目のインデックスを取得.
							while ((idx = ListView_GetNextItem(hList, idx, LVNI_SELECTED)) != -1){	// ListView_GetNextItemで選択項目のインデックス取得.(引数のidxが-1の時は最初の選択要素のインデックスを戻り値のidxに返し, そのidxでその次のインデックスを取得できる.これが-1になるまで繰り返される.)
								
								// インデックスの値を表示.
								_stprintf(tszIdx, _T("idx = %d\n"), idx);	// idxを文字列に変換.
								MessageBox(NULL, tszIdx, _T("ListView_GetNextItem"), MB_OK);	// MessageBoxでtszIdxを表示.
