...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ShowCaret");						// ウィンドウクラス名は"ShowCaret".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass Failure!"), _T("ShowCaret"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass Failure!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("ShowCaret"), _T("ShowCaret"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"ShowCaret"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow Failure!"), _T("ShowCaret"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow Failure!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
	...
	switch (uMsg){	// switch-casa文でuMsgの値ごとに処理を振り分ける.
		...
		case WM_CREATE:		// ウィンドウの作成が開始された時.(uMsgがWM_CREATEの時.)
			...
			// WM_CREATEブロック
			{
				...
				// エディットコントロールの作成.
				hEdit = CreateWindow(WC_EDIT, _T("Edit1"), WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | ES_WANTRETURN | ES_MULTILINE | ES_AUTOHSCROLL | ES_AUTOVSCROLL, 50, 50, 500, 100, hwnd, (HMENU)(WM_APP + 1), lpCS->hInstance, NULL);	// CreateWindowのWC_EDITでエディットコントロール"Edit1"を作成.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目のIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_2_1:
						...
						// ID_ITEM_2_1ブロック
						{
							...
							MessageBox(hwnd, tszText, _T("ShowCaret"), MB_OK);	// tszTextを表示.
					...
					case ID_ITEM_2_2:
						...
						// ID_ITEM_2_2ブロック
						{
							...
							MessageBox(hwnd, tszPoint, _T("ShowCaret"), MB_OK);	// tszPointを表示.
					...
					case ID_ITEM_3_1:
						...
						// ID_ITEM_3_1ブロック
						{
							...
							MessageBox(hwnd, tszPos, _T("ShowCaret"), MB_OK);	// tszPosを表示.
							...
							MessageBox(hwnd, tszResult, _T("ShowCaret"), MB_OK);	// tszResultを表示.
					...
					case ID_ITEM_4_1:
						...
						// ID_ITEM_4_1ブロック
						{
							...
							SetWindowText(hEdit, tszPath);	// tszPathをhEditにセット.
					...
					case ID_ITEM_5_1:
						...
						// ID_ITEM_5_1ブロック
						{
							...
							LONG lStart = 0;	// 開始位置
							LONG lEnd = 0;		// 終了位置
							SendMessage(hEdit, EM_GETSEL, (WPARAM)&lStart, (LPARAM)&lEnd);	// SendMessageにEM_GETSELで選択範囲を取得.
							TCHAR tszResult[128] = {0};	// TCHAR文字配列tszResult(長さ128)を{0}で初期化.
					...
					case ID_ITEM_5_2:
						...
						// ID_ITEM_5_2ブロック
						{
							...
							SendMessage(hEdit, EM_SETSEL, (WPARAM)0, (LPARAM)2);	// SendMessageにEM_SETSELで選択範囲を設定.
					...
					case ID_ITEM_5_3:
						...
						// ID_ITEM_5_3ブロック
						{
							...
							SendMessage(hEdit, EM_REPLACESEL, (WPARAM)FALSE, (LPARAM)_T("XYZ"));	// SendMessageで選択範囲のテキストを"XYZ"に置換.
					...
					case ID_ITEM_6_1:
						...
						// ID_ITEM_6_1ブロック
						{
							...
							// キャレットを非表示にする.
							HideCaret(hEdit);	// HideCaretでhEditのキャレットを非表示.
					...
					case ID_ITEM_6_2:
						...
						// ID_ITEM_6_2ブロック
						{
							...
							// キャレットを表示にする.
							ShowCaret(hEdit);	// ShowCaretでhEditのキャレットを表示.
