...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd){
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("EN_UPDATE");							// ウィンドウクラス名は"EN_UPDATE".
	...
	if (!RegisterClass(&wc)){	// RegisterClassでウィンドウクラスを登録し, 0が返ったらエラー.
		...
		// エラー処理
		MessageBox(NULL, _T("RegisterClass failed!"), _T("EN_UPDATE"), MB_OK | MB_ICONHAND);	// MessageBoxで"RegisterClass failed!"とエラーメッセージを表示.
		return -1;	// 異常終了(1)

	}

	// ウィンドウの作成
	hWnd = CreateWindow(_T("EN_UPDATE"), _T("EN_UPDATE"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで, 上で登録した"EN_UPDATE"ウィンドウクラスのウィンドウを作成.
	if (hWnd == NULL){	// ウィンドウの作成に失敗したとき.

		// エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("EN_UPDATE"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"とエラーメッセージを表示.
		return -2;	// 異常終了(2)
		...
		case WM_CLOSE:	// ウィンドウが閉じられた時.(uMsgがWM_CLOSEの時.)
			...
			// WM_CLOSEブロック
			{
				...
				if (bRet){	// TRUEなら.
					...
					// "Modify!"と表示.
					MessageBox(hwnd, _T("Modify!"), _T("EN_UPDATE"), MB_OK | MB_ICONASTERISK);	// MessageBoxで"Modify!"と表示.
				...
				switch (LOWORD(wParam)){	// LOWORD(wParam)で選択されたメニュー項目, またはコントロールのIDが取得できるので, その値で判定する.
					...
					case ID_ITEM_RESET_MODIFY:
						...
						// ID_ITEM_RESET_MODIFYブロック
						{
							...
							// ウィンドウタイトルを元に戻す.
							SetWindowText(hwnd, _T("EN_UPDATE"));	// SetWindowTextでhwndのウィンドウタイトルを"EN_UPDATE"に戻す.
					...
					// ID_EDIT1で何らかの操作が行われた時.
					case ID_EDIT1:

						// ID_EDIT1ブロック
						{

							// エディットコントロールで, 入力内容が変更された直後の通知コードはEN_UPDATE( == 0x0400).
							if (HIWORD(wParam) == EN_UPDATE){	// HIWORD(wParam)はEN_UPDATE( == 0x0400).
								SetWindowText(hwnd, _T("Modified!"));	// SetWindowTextでhwndのウィンドウタイトルを"Modified!"に変更.
							}
