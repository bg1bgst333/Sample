...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	// メッセージ処理ループ(Windowsのtelnetは1文字キーを押すごとに送信してしまうのでそれに対応した特殊なバージョン.)
	while (!iExitFlag){	// iExitFlag == 1なら, このループから抜ける.
		...
		// qなら抜ける.
		if (strcmp((char *)btBuf, "q") == 0){	// strcmpで比較して"q"なら.

			// 抜ける.
			iExitFlag = 1;	// iExitFlagを1にする.

		}
		else if ((char)btBuf[0] == '\r' && (char)btBuf[1] == '\n'){	// 改行だった場合.

			// 改行は表示しないで消す.
			btBuf[0] = 0x0;	// 0番目に0x0をセット.
			btBuf[1] = 0x0;	// 1番目に0x0をセット.
