...
DWORD WINAPI ThreadProc(LPVOID lpParameter){
	...
	HANDLE hMutex;	// HANDLE型ミューテックスハンドル変数hMutex.

	// 引数lpParameterをTHREAD_PARAM型のポインタにキャスト.
	pTP = (THREAD_PARAM *)lpParameter;	// lpParameterをTHREAD_ARG型のポインタにキャストして, pTPに格納.

	// ミューテックスを開く.
	hMutex = OpenMutex(MUTEX_ALL_ACCESS, FALSE, MUTEX_NAME);	// OpenMutexでミューテックスMUTEX_NAME("Mutex")を開く.

	// ミューテックスが解放されるまで待つ.
	WaitForSingleObject(hMutex, INFINITE);	// WaitForSingleObjectでミューテックスが解放されるまで待つ.
	...
	// ミューテックスの解放.
	ReleaseMutex(hMutex);	// ReleaseMutexでミューテックスを解放.

	// ミューテックスハンドルを閉じる.
	CloseHandle(hMutex);	// CloseHandleでhMutexを閉じる.
...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	// ミューテックスの作成
	hMutex = CreateMutex(NULL, FALSE, MUTEX_NAME);	// CreateMutexでMUTEX_NAME("Mutex")なミューテックスを作成.
