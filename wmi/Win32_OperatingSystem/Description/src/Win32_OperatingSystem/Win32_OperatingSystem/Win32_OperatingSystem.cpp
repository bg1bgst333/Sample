...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	BSTR strQuery = SysAllocString(L"SELECT * FROM Win32_OperatingSystem");	// SysAllocStringでstrQueryを"SELECT * FROM Win32_OperatingSystem"で初期化.
	...
	while (pEnumerator){	// pEnumeratorがNULLで無い限り続ける.
		...
		if (SUCCEEDED(hr7)){	// SUCCEEDEDマクロで判定.
			...
			VARIANT vrVersion;	// バージョン.
			hr9 = pClassObject->Get(L"Version", 0, &vrVersion, 0, 0);	// IWbemClassObject::Getで"Version"プロパティの値を取得.
			if (SUCCEEDED(hr9)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"Version: %s\n", vrVersion.bstrVal);	// vrVersion.bstrValを出力.
				VariantClear(&vrVersion);	// VariantClearでvrVersionをクリア.
			}
			VARIANT vrCaption;	// キャプション.
			hr10 = pClassObject->Get(L"Caption", 0, &vrCaption, 0, 0);	// IWbemClassObject::Getで"Caption"プロパティの値を取得.
			...
			VARIANT vrDescription;	// 説明.
			hr11 = pClassObject->Get(L"Description", 0, &vrDescription, 0, 0);	// IWbemClassObject::Getで"Description"プロパティの値を取得.
			if (SUCCEEDED(hr11)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"Description: %s\n", vrDescription.bstrVal);	// vrDescription.bstrValを出力.
				VariantClear(&vrDescription);	// VariantClearでvrDescriptionをクリア.
			}
		}
