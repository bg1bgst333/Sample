...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	BSTR strQuery = SysAllocString(L"SELECT * FROM Win32_PnPEntity WHERE PNPDeviceID LIKE \"USB%\"");	// SysAllocStringでstrQueryを"SELECT * FROM Win32_PnPEntity WHERE PNPDeviceID LIKE \"USB%\""で初期化.
	...
	while (pEnumerator){	// pEnumeratorがNULLで無い限り続ける.
		...
		if (SUCCEEDED(hr7)){	// SUCCEEDEDマクロで判定.
			...
			hr10 = pClassObject->Get(L"DeviceID", 0, &vrDeviceID, 0, 0);	// IWbemClassObject::Getで"DeviceID"プロパティの値を取得.
			...
			VARIANT vrPNPDeviceID;	// PNPデバイスID.
			hr11 = pClassObject->Get(L"PNPDeviceID", 0, &vrPNPDeviceID, 0, 0);	// IWbemClassObject::Getで"PNPDeviceID"プロパティの値を取得.
			if (SUCCEEDED(hr11)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				//_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				if (vrPNPDeviceID.vt != VT_NULL){	// VT_NULLになることがあるようなので, この判定が必要.
					wprintf(L" PNPDeviceID: %s\n", vrPNPDeviceID.bstrVal);	// vrPNPDeviceID.bstrValを出力.
				}
				VariantClear(&vrPNPDeviceID);	// VariantClearでvrPNPDeviceIDをクリア.
			}
		}
