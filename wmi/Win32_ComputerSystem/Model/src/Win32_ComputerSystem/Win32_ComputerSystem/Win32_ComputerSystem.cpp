...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	HRESULT hr10;	// IWbemClassObject::Getの結果を格納するhr10.
	HRESULT hr11;	// IWbemClassObject::Getの結果を格納するhr11.
	...
	BSTR strQuery = SysAllocString(L"SELECT * FROM Win32_ComputerSystem");	// SysAllocStringでstrQueryを"SELECT * FROM Win32_ComputerSystem"で初期化.
	...
	while (pEnumerator){	// pEnumeratorがNULLで無い限り続ける.
		...
		if (SUCCEEDED(hr7)){	// SUCCEEDEDマクロで判定.
			...
			VARIANT vrUserName;	// ユーザー名.
			hr9 = pClassObject->Get(L"UserName", 0, &vrUserName, 0, 0);	// IWbemClassObject::Getで"UserName"プロパティの値を取得.
			if (SUCCEEDED(hr9)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"UserName: %s\n", vrUserName.bstrVal);	// vrUserName.bstrValを出力.
				VariantClear(&vrUserName);	// VariantClearでvrUserNameをクリア.
			}
			VARIANT vrDomain;	// ドメイン.
			hr10 = pClassObject->Get(L"Domain", 0, &vrDomain, 0, 0);	// IWbemClassObject::Getで"Domain"プロパティの値を取得.
			if (SUCCEEDED(hr10)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"Domain: %s\n", vrDomain.bstrVal);	// vrDomain.bstrValを出力.
				VariantClear(&vrDomain);	// VariantClearでvrDomainをクリア.
			}
			VARIANT vrModel;	// モデル.
			hr11 = pClassObject->Get(L"Model", 0, &vrModel, 0, 0);	// IWbemClassObject::Getで"Model"プロパティの値を取得.
			if (SUCCEEDED(hr11)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"Model: %s\n", vrModel.bstrVal);	// vrModel.bstrValを出力.
				VariantClear(&vrModel);	// VariantClearでvrModelをクリア.
			}
		}
