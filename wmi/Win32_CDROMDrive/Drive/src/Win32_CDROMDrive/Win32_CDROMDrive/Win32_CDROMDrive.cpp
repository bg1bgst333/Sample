...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	BSTR strQuery = SysAllocString(L"SELECT * FROM Win32_CDROMDrive");	// SysAllocStringでstrQueryを"SELECT * FROM Win32_CDROMDrive"で初期化.
	...
	while (pEnumerator){	// pEnumeratorがNULLで無い限り続ける.
		...
		if (SUCCEEDED(hr7)){	// SUCCEEDEDマクロで判定.
			...
			VARIANT vrDrive;	// ドライブ.
			hr10 = pClassObject->Get(L"Drive", 0, &vrDrive, 0, 0);	// IWbemClassObject::Getで"Drive"プロパティの値を取得.
			if (SUCCEEDED(hr10)){	// SUCCEEDEDマクロで判定.
				// pClassObject->Getは成功.
				_tprintf(_T("pClassObject->Get OK!\n"));	// "pClassObject->Get OK!"と出力.
				wprintf(L"Drive: %s\n", vrDrive.bstrVal);	// vrDrive.bstrValを出力.
				VariantClear(&vrDrive);	// VariantClearでvrDriveをクリア.
			}
		}
