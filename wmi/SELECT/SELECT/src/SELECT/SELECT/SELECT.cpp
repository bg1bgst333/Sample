...
int _tmain(int argc, TCHAR *argv[]){	// main関数のTCHAR版.
	...
	BSTR strQuery = SysAllocString(L"SELECT Name, DeviceID FROM Win32_USBHub WHERE Name = \"USB 大容量記憶装置\"");	// SysAllocStringでstrQueryを"SELECT Name, DeviceID FROM Win32_USBHub WHERE Name = \"USB 大容量記憶装置\""で初期化.
	...
	while (pEnumerator){	// pEnumeratorがNULLで無い限り続ける.
		...
		if (SUCCEEDED(hr7)){	// SUCCEEDEDマクロで判定.
			...
			if (SUCCEEDED(hr9)){	// SUCCEEDEDマクロで判定.
				...
				// pClassObject->Getは失敗.
				_tprintf(_T("pClassObject->Get NG!\n"));	// "pClassObject->Get NG!"と出力.
				if (hr9 == WBEM_E_NOT_FOUND){	// hr9がWBEM_E_NOT_FOUNDの時.
					_tprintf(_T("WBEM_E_NOT_FOUND\n"));	// "WBEM_E_NOT_FOUND"と出力.
				}
			}
