...
// メンバの定義
// 引数付きコンストラクタclass_term(std::string str)
class_term::class_term(std::string str){
  ...
  // 演算子が無い.
  if (inner.find("+") == -1 && inner.find("-") == -1 && inner.find("*") == -1 && inner.find("/") == -1){
    operand1_str = inner;
  }  
  else{
    ...
    if (!muldiv){ // muldivがない.
      ...
      operand1_ = new class_expression(operand1_str); // opereand1_を生成.
    }
  }
