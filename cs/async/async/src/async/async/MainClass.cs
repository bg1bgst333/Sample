...
class MainClass
{
    ...
    static async void SimpleMethod()    // awaitのあるメソッドにはasyncを付けないといけない.
    {
        ...
        await Task.Run(async () =>    // awaitでこの処理を待つ.(ラムダ式にasyncを付けて非同期ラムダ式.)
        {
            ...
            // LongProcMethodを実行.
            await LongProcMethod();   // awaitを指定してLongProcMethodを呼ぶ.
    ...
    // LongProcMethodの定義
    static async Task LongProcMethod()  // async Taskにする.
    {
