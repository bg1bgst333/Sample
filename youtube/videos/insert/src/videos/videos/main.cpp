...
int _tmain() {
	...
	// POSTリクエストヘッダの作成
	request_tstr = _T("POST /upload/youtube/v3/videos?part=status HTTP/1.0");	// request_tstrに"POST /youtube/v3/video?part=snippet&uploadType=direct HTTP/1.0"を格納.
	...
	//request_tstr = request_tstr + _T("\r\n");	// 改行
	request_tstr = request_tstr + _T("Content-Type: video/*");	// request_tstrに"Content-Type: application/octet-stream"を連結.
	request_tstr = request_tstr + _T("\r\n");	// 改行
	...
	// 空行
	/*
	memset(request_str, 0, sizeof(char) * 4096);	// クリア.
	request_tstr = _T("\r\n\r\n");	// 空行
	len = WideCharToMultiByte(CP_UTF8, 0, request_tstr.c_str(), -1, NULL, 0, NULL, NULL);	// マルチバイトに変換するためのサイズを取得.
	WideCharToMultiByte(CP_UTF8, 0, request_tstr.c_str(), -1, request_str, len, NULL, NULL);	// マルチバイトに変換.
	SSL_write(ssl, request_str, 4);	// SSL_writeでrequest_strを書き込む.
	*/
