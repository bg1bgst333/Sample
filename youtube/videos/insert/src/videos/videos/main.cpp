...
int _tmain() {
	...
	tstring body_tstr;	// リクエストボディ文字列body_tstr.
	int content_length = 0;	// ボディの長さcontent_length.
	TCHAR content_length_tstr[32] = { 0 };	// content_lengthを文字列にしたものcontent_length_tstrを0で初期化.
	HANDLE hFile;	// HANDLE型ファイルハンドルhFile
	DWORD dwSize;	// DWORD型ファイルサイズdwSize
	TCHAR bin_size_tstr[32] = { 0 };	// dwSizeを文字列にしたものbin_size_tstrを0で初期化.
	...
	// dwSizeの変換.
	_stprintf_s(bin_size_tstr, _T("%lu"), dwSize);	// dwSizeを文字列変換.

	// リクエストボディの作成.
	body_tstr = _T("{");	// body_tstrに"{"を格納.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("  \"snippet\": {");	// body_tstrに""  \"snippet\": {""を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("    \"title\": \"");	// body_tstrに"    \"title\": \""を連結.
	body_tstr = body_tstr + UPLOAD_FILENAME;	// body_tstrにUPLOAD_FILENAMEを連結.
	body_tstr = body_tstr + _T("\",");	// body_tstrに"\","を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("    \"description\": \"videos/insert\",");	// body_tstrに"\"description\": \"videos/insert\","を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("    \"tags\": [\"test\", \"video\"],");	// body_tstrに"\"tags\": [\"test\", \"video\"],"を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("    \"categoryId\": 22");	// body_tstrに"\"categoryId\": 22"を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("  }");	// body_tstrに"  }"を連結.
	body_tstr = body_tstr + _T("\r\n");	// 改行
	body_tstr = body_tstr + _T("}");	// body_tstrに"}"を連結.
	...
	content_length = len - 1;	// len - 1がcontent_length.
	_stprintf_s(content_length_tstr, _T("%lu"), content_length);	// content_lengthを文字列変換.

	// リクエストの作成(ヘッダを作成し, 上記のボディを連結.
	request_tstr = _T("POST /upload/youtube/v3/videos?part=snippet&uploadType=resumable HTTP/1.0");	// request_tstrに"POST /upload/youtube/v3/videos?part=snippet&uploadType=resumable HTTP/1.0"を格納.
	...
	request_tstr = request_tstr + _T("\r\n");	// 改行
	request_tstr = request_tstr + _T("Content-Type: application/json; charset=UTF-8");	// request_tstrに"Content-Type: application/json; charset=UTF-8"を連結.
	request_tstr = request_tstr + _T("\r\n");	// 改行
	...
	request_tstr = request_tstr + _T("\r\n");	// 改行
	request_tstr = request_tstr + _T("X-Upload-Content-Type: application/octet-stream");	// request_tstrに"X-Upload-Content-Type: application/octet-stream"を連結.
	request_tstr = request_tstr + _T("\r\n");	// 改行
	request_tstr = request_tstr + _T("X-Upload-Content-Length: ");	// request_tstrに"X-Upload-Content-Length: "を連結.
	request_tstr = request_tstr + bin_size_tstr;	// request_tstrにbin_size_tstrを連結.
	request_tstr = request_tstr + _T("\r\n\r\n");	// 空行
