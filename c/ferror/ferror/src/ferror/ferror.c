...
int main(void){
  ...
  int written_len = 0; /* 書き込めた長さwritten_lenを0で初期化. */
  ...
  while (1){ /* 無限ループ */
    ...
    if (read_len < 3){ /* 3よりも小さい時. */
      ...
      /* "XYZ"を書き込めるか試してみる. */
      written_len = fwrite("XYZ", 1, 3, fp); /* fwriteで"XYZ"を書き込む. */
      printf("written_len = %d\n", written_len); /* written_lenの出力. */
      perror("fwrite"); /* "fwrite"のエラー出力. */
      if (ferror(fp)){ /* ferrorが真なら. */
        printf("ferror true!\n"); /* "ferror true"を出力. */
        break; /* いち早くループを抜ける. */
      }
    }
